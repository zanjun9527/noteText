...
红黑树，map中好处在哪
  ...
  平衡二叉树和红黑树区别
    ...
    用红黑树的原因,
      类似1，4，5，7，8，9,用二叉树排出来,就像一条右单链表,时间复杂度就变成了O(N),这是引入红黑树可以缩短树的高度,效率查询
      可以说,红黑树就是用来优化链表结构的
...
常用配置参数
  ...
  timeUnit: 时间单位
  blockingQueue: 阻塞队列,当线程数达到核心线程数时，任务会在队列中等待，若等待任务数大于指定队列大小，则开启非核心线程进行执行任务, 最大maxPoolSize个
...
设计一个线程池的各个参数，各种场景下线程池的设计
  CPU密集型，最大线程数是CPU的核数，IO密集型，最大线程数是CPU的核数的两倍


  N为CPU总核数，linux下使用lscpu命令查看下即可
  CPU总核数 = 物理CPU个数 * 每颗物理CPU的核数，总逻辑CPU数 = 物理CPU个数 * 每颗物理CPU的核数 * 超线程数。

  如果是CPU密集型应用，则线程池大小设置为N+1 ，一般来说：计算型代码、Bitmap转换、Gson转换等
  如果是IO密集型应用，则线程池大小设置为2N+1，一般来说：文件读写、DB读写、网络请求等

  一个核心只能同时执行一个线程

  比如，Linux服务器有4个CPU，每个CPU有4个核心，每个核心有4个超线程，一共有4*4*4=64个线程，也称为64个逻辑cpu。
...
常见的锁类型，自旋锁，偏向锁，轻量级锁，可重入锁，公平锁，非公平锁，乐观锁，悲观锁
  ...
  锁优化，
    偏向锁(单线程) => 轻量级锁(适量自旋) => 重量级锁

    重量级锁，所有来加锁的线程加锁失败之后，就会加入等待的队列中，等待别的线程释放锁之后再进行锁的竞争。
    synchronized在最开始时并不是上来就是一把重量级锁，而是随着多线程并发竞争锁的激烈程度的提高来不断的升级，慢慢变成重量级锁，在整个升级的过程会经历偏向锁、轻量级锁、自旋锁、重量级锁的过程。

实现分布式锁

AbstractQueuedSynchronized（AQS）
      重点是状态变量state，同步队列Sync queue(FIFO，Node节点组成)
      tryAcquire()，tryRelease()方法加锁，释放锁
...
#### 分布式锁
    ...
    主要要解决的几个问题
      ...
      redis限流
          redisson限流器RRateLimiter.trySetRate(),5秒中产生3个令牌,令牌桶法
          redis利用滑动区间时间窗口实现，zset结构，zremrangeByScore

        * redis的zset限流(底层是跳表结构)
            zadd key score value			向zset有序集合里添加元素
            zremrangeByScore key min max	移除有序集中，指定分数（score）区间内的所有成员
            
            通过zadd命令往有序集合里存放数据时引入表示权重的score参数，并在其中存放时间戳。由于zset是有序集合，因此包含时间戳的score能被排序
            通过zremrangeByScore命令能在有序集合里删除键为key、score值在min到max范围内的数据。排除限流时间范围外的数据，能通过zcard命令统计有序集合内元素的数量，以确保请求数量小于限流的上限值
            ...
            class LimitUtil{
              ...
              public static void canVisit(Jedis jedis,String requestType,int limitTime,int limitNum){
                ...
                //统计时间范围内总数
                Long count=jedis.zcard(requestType);//检测最近10s的请求数
                //设置所有请求的超时时间
                jedis.expire(requestType,limitTime+1);
                boolean flag=limitNum>=count;
                if(flag){
                  System.out.println("Can visit");
                  //Thread.sleep(2000);	//测试用，不然zadd一直往里面放请求，zcard的count会数量一直增大
                }else{
                  System.out.println("Can not visit");
                  //Thread.sleep(4000);   //测试用，
                }
              }
            }
            
            通过zremrangeByScore命令去除0到距当前时间limitTime时间范围内的数据。比如限流的时间范围是100秒，那么通过zremrangeByScore命令就能在有序集合里去除score范围从0到距离当前时间100秒的数据，这样就能确保有序集合内只存有最近100秒内发来的元素。
            用zcard命令统计有序集合内键为requestType的个数，判断当前个数是否达到限流的上限，
            expire超时时间，限流动作完成后这些键能自动删除
            
            
            滑动窗口的效果，并且能保证每N秒内至多M个请求，缺点就是zset的数据结构会越来越大
            redis的封装操作还是要注意线程安全，需保证添加和判断一块是原子性的操作才行
            ...
            针对ip也可以限制，zadd(ipstr, currentTimeMillis, "" + UUID.randomUUID())
            ```
              ...
              // ipAddress Ip地址	period    特定的时间内(s)	 maxCount  最大允许的次数
        ...
        redis的list
          文章列表或者数据分页展示的应用。
          Redis的列表，列表不但有序同时还支持按照范围内获取元素，可以完美解决分页查询功能。大大提高查询效率。
...
## boot相关
    自动配置原理
      ...
      *****
        启动自动装配流程
          思路是启动后类启动，自动加载那些配置(spring.factories)，那些需要排除自定义(@conditionalOnClass等注解)
      ...
      *****
      ...
      SpringBoot 的自动加载机制类似于 JavaSPI加载机制（框架面向接口开发，具体实现留给第三方或者扩展实现，框架通过扫描这些实现进行第三方服务或扩展的调用），
      SpringBoot通过读取在jar包的META-INF下的spring.factories读取加载类信息。 而spring.factories实质上是一个properties文件。

      *****
        spring.factories在autoconfigure包和spring-boot-2.3.2.RELEASE.jar下都有
        @conditionalOnClass等条件注解在autoconfigure的jar下
      *****
...
类加载 双亲委派(向上递归检查，向下递归加载),一个类只能被一个类加载器加载
  ...
  loadClass() 和 forName() 的区别	
    ...
    loadClass()方法获得的Class对象只完成了类加载过程中的第一步：加载，后续的操作均未进行
    ClassLoader.loadClass加载的类不进行解析操作，不进行解析操作就意味着初始化也不会进行，那么其类的静态参数就不会初始化，静态代码块也不会被执行。						

    Class.forName()方法获得 Class 对象是已经执行完初始化（类加载最后一步：初始化，而非是实例化对象操作的初始化）
...
## 分布式
  ...
  ### 服务化的好处，减压，公用，解耦
        ...
        几种熔断方案的对比，https://www.icode9.com/content-4-620551.html
        ...
        熔断，降级，限流 
          ...
          调研的限流框架大概有
            ...
            4、其他：redission、手撸代码
            实际需求情况属于业务端限流，redission更加方便，使用更加灵活。

            
          限流
          1、使用guava提供工具库里的RateLimiter类(内部采用令牌捅算法实现)进行限流，分布式不适用
          2、使用Java自带Semaphore信号量实现，分布式不适用
          ...
          令牌桶算法(Token Bucket)和 Leaky Bucket
            令牌桶算法和漏桶算法效果一样但方向相反的算法，更加容易理解.随着时间流逝,系统会按恒定1/QPS时间间隔(如果QPS=100,则间隔是10ms)往桶里加入Token(想象和漏洞漏水相反,有个水龙头在不断的加水),如果桶已经满了就不再加了.新请求来临时,会各自拿走一个Token,如果没有Token可拿了就阻塞或者拒绝服务.

            令牌桶需要一个ScheduledThread不断的放入令牌，

          算法选择
            漏桶算法与令牌桶算法的区别在于，漏桶算法能够强行限制数据的传输速率，令牌桶算法能够在限制数据的平均传输速率的同时还允许某种程度的突发情况。
            令牌桶还有一个好处是可以方便的改变速度。一旦需要提高速率，则按需提高放入桶中的令牌的速率。所以，限流框架的核心算法还是以令牌桶算法为主


          实际项目使用
            dubbo自带的mock功能实现降级处理（可以直接return 串 或者自定义mock处理类），然后需要熔断的话可以引入sentinel，接入控制台dashboard，配置流控和降级，可以自定义fallback处理方法
            sentinel的主要是qps流控，和慢调用比例，
            然后hystrix还是属于cloud的那套熔断框架，类似fallback处理方法
            ...
            一般的流控或者降级类异常进入blockHandler，其他异常进入fallback处理

            @SentinelResource注解，需要引入jar
              <dependency>
  ...
  #### dubbo的服务降级和熔断
            ...
            Dubbo 本身就提供了服务降级的机制；而 Dubbo 的服务降级机制主要是利用服务消费者的 mock 属性
            mock主要有两种配置方式，
            1.直接返回简单结果
              在远程调用异常时，服务端直接返回一个固定的字符串(也就是写死的字符串)
              消费端配置mock参数：	<dubbo:reference id="xxxService" interface="com.x..service.xxxxService" check="false" mock="return 123456" />
            ...
            2.自定义mock处理类
              ...
              在消费端配置mock参数：<dubbo:reference id="xxxService" interface="com.x..service.xxxxService" check="false" mock="true" />

              说明：配置了mock参数之后，假设在调用服务的时候，远程服务没有启动，或者各种网络异常了，那远程服务会去寻找自定义的mock业务处理类进行业务处理。
              因此还需配置一个自定义mock业务处理类，实测没有加入注解扫描也可以执行。
...
## mysql相关
    ...
    sql优化主要就是针对走不走索引的优化，一般函数，in，null列，%开头，<>等都不走索引
        以%开头的LIKE语句，模糊匹配
        OR 语句前后没有同时使用索引
        数据类型出现隐式转化（如varchar不加单引号的话可能会自动转换为int型）
        where 子句中对字段进行表达式操作或者函数操作
...
NIO     
    ...
    *** 一个线程通过一个selector，控制多个socket的读写，多路复用***
...
NIO 主要有三大核心部分：Channel(通道)，Buffer(缓冲区), Selector。channel下面是buffer
...
Redis和Mysql如何保证数据一致(双写一致)
      延时双删，
        步骤是：先删除Redis缓存数据，再更新Mysql，延迟几百毫秒再删除Redis缓存数据，这样就算在更新Mysql时，有其他线程读了Mysql，把老数据读到了Redis中，那么也会被删除掉，从而把数据保持一致。删redis-正更新数据库，这个期间会有其他线程读取旧的数据去填入redis，这里会有问题

      	读的时候，先读缓存，缓存没有的话，就读数据库，然后取出数据后放入缓存，同时返回响应。
		    写的时候，先删除cache，再更新数据库，然后再删除缓存。(双删，休眠100毫秒，再次淘汰缓存)

      mq订阅(忽略)
		    mysql Binlog增强订阅消费+消息队列+增量数据更新到redis—读redis：热数据基本上都在redis—写mysql：增删改都是操作mysql—更新redis数据：mysql的数据操作Binlog，来更新redis
...
Java如何保证线程安全，互斥同步+非阻塞同步(无需线程挂起)，悲观锁和乐观锁两种思路
...
类加载，加载的几个阶段：加载，验证，准备，解析，初始化，使用，卸载，前5个就是类的加载过程，2-4属于连接
  ...
  加载，验证，准备，解析，初始化
    ...
    * 解析：
          将符号引用转化为直接引用（指针引用）
    * 初始化：
          执行一些静态代码块，为静态变量赋值，这里的赋值才是代码里面的赋值



对象不可达 GC Root 时，这个对象并不会立马被回收，若要被真正的回收需要经历两次标记
第一次标记并且进行一次筛选，筛选的条件是是否有必要执行 finalize()方法(二次标记)。
...
### 1. 111
      ...
      redis主从，复制断电，数据丢失，
        ...
        1.主从复制-备份
            备份或者读写分离。
              复制的是rdb的快照，过程中有复制的缓冲区，主库完成RDB文件发送后，再把缓冲区中修改操作发送给从库
              主节点bgsave命令fork子进程进行RDB持久化
        ...
        哨兵模式
          ...
          脑裂问题，网络问题，导致临时2个m，解决配置：m断开s10秒，拒绝写入，可配置
          从节点的意义在于故障转移的能力
...
### 2.111
      ...
      redis日排行
        ...
        zSet排序，数据存在hSet中。Zset根据score 排序，查询出value（相当于hSet中的key）,再从Hset中获取相应的值。
        ...
        排序：zrange 2kshoot 0 2 withscores        
              倒序zrevrange ，withscores为输出结果带分数,zrange范围是左开右闭,返回有序集合中排名介于[start,  stop]之间的成员

        ZRANGEBYSCORE全称zset range by score       返回有序集合中分值介于min和max之间的所有成员
      ...
      Threadlocal的泄漏问题
        ...
        ***本质即使ThreadLocalMap中的entry结构是<ThreadLocal,Value>,对应的是(WeakReference,value),jvm回收key后变成了(null,value),导致内存泄漏***

        每个Thread内部维护一个私有属性:ThreadLocal.ThreadLocalMap threadLocals。
        ThreadLocalMap内部结构为KV键值对<ThreadLocal,Value>,ThreadLocal本身不存储线程的变量值，只是一个工具，用来维护Thread内部Map，帮助存和取。

        弱引用的特点是，如果这个对象只存在弱引用，那么在下一次垃圾回收的时候必然会被清理掉
        java中的弱引用在内存不足的时候会被回收掉，回收之后变成(null,value)的形式，key被收回掉了,key为空的话value是无效数据,value却不一定能够被回收，因为他还与Current Thread存在一个强引用关系

        Entry继承了WeakReference<ThreadLocal<?>>,即Entry的key是弱引用,map中是(WeakReference,value)数据结构。
        由于ThreadLocalMap的key是ThreadLocal对象的弱引用，而Value是强引用。如果创建ThreadLocal的线程一直持续运行，那么这个Entry对象中的value就有可能一直得不到回收，发生内存泄露。


        ThreadLocal 在 ThreadLocalMap 中是以一个弱引用身份被Entry中的Key引用的，结构是(WeakReference,value),
        key,ThreadLocal会在下次JVM垃圾收集时被回收,出现Entry中<null,value>,外部读取ThreadLocalMap中的元素是无法通过null Key来找到Value的。
        value对象便被ThreadLocalMap引用着，如果当前线程的生命周期很长，一直存在，那么其内部的ThreadLocalMap对象也一直生存下来，
        这些null key就存在一条强引用链的关系一直存在：Thread --> ThreadLocalMap–>Entry–>Value，
        这条强引用链会导致Entry不会回收，Value也不会回收，但Entry中的Key却已经被回收的情况，造成内存泄漏。

        JVM团队已经优化，并做了一些措施来保证ThreadLocal尽量不会内存泄漏：
        在ThreadLocal的get()、set()、remove()方法调用的时候会清除掉线程ThreadLocalMap中所有Entry中Key为null的Value，并将整个Entry设置为null，利于下次内存，

        ThreadLocalMap的key设计成弱引用的原因
          如果key设计成强引用且没有手动remove()，那么key会和value一样伴随线程的整个生命周期，
          如果key是弱引用，被GC后至少ThreadLocal被回收了，在下一次的set()、get()、remove()还会回收key为null的Entry的value。
...
### 3.111
      ...
      订单超时取消，mq延迟队列
          实现场景
            A用户下单之后迟迟没有付款，那只要A用户不付款，这个库存就一直属于该用户，这里需要订单超时自动取消释放库存
            比如电商里，提交了一个订单就可以发送一个延时消息，1h后去检查这个订单的状态，如果还是未付款就取消订单释放库存。

          rocketmq实现延迟队列，只需要，message.setDelayTimeLevel(9);这里设置延迟的级别即可
          只需要保证这个消息的发送能按用户设置的延时级别准时投递给消费者即可，至于释放库存这一逻辑，那是你业务上的事情。
          在消费到延时消息后，检查订单状态是否已经完成，如果用户没有支付的话则将订单设置为超时，然后将库存还回去。

          延迟级别(分别对应级别，1 2 3...)，默认支持18个level的延迟消息
            1s 5s 10s 30s 1m 2m 3m 4m 5m 6m 7m 8m 9m 10m 20m 30m 1h 2h   

          实现原理
            Broker启动后，会创建一个内部主题：SCHEDULE_TOPIC_XXXX，根据延迟level的个数(18level对应18队列)，创建对应数量的队列
            这里这并不是说这个内部主题只会有18个队列，因为Broker通常是集群模式部署的，因此每个节点都有18个队列。

            延时消息其核心就是两点
              替换消息的topic为特定延时消息topic，queueId为delayLevel-1
              备份消息原有的topic，queueId，方便后面从新取出进行消息投递
              
            1.producer端设置消息delayLevel延迟级别，消息属性DELAY中存储了对应了延时级别
            2.broker端收到消息后，判断延时消息延迟级别，若是大于0，则备份消息原始topic，queueId，并将消息topic改成延时消息队列特定topic(SCHEDULE_TOPIC)，queueId改成延时级别-1
            3.mq服务端ScheduleMessageService中，为每个延迟级别单独设置一个定时器，定时(每隔1秒)拉取对应延迟级别的消费队列
            4.根据消费偏移量offset从commitLog中解析出对应消息
            5.从消息tagsCode中解析出消息应当被投递的时间，与当前时间作比较，判断是否应该进行投递
            6.若到达了投递时间，则构建一个新的消息，并从消息属性中恢复出原始的topic，queueId，并清除消息延迟属性，重新进行消息投递

            消费者消费消息是根据其订阅的Topic中有没有消息来决定的，而RocketMQ就是把你需要延时消费的消息暂存在另一个队列，直到时间到了再投递给你想要的Topic去消费。
            定时消息会暂存在，SCHEDULE_TOPIC_XXXX中(修改该消息主题)，队列queueId=delayLevel-1，这样给一个queue只存相同延迟的消息(顺序消费)，broker会调度消费SCHEDULE_TOPIC_XXXX，将其存入真是的topic中
      ...
      手工mq实现，jmc实现？
        1. 基于多线程的方式实现MQ(极简纯享版)
          ...
          这里要注意的是不能用普通队列，必须用阻塞队列，因为阻塞队列当队列有东西的时候回自动触发消费者线程从队列里取东西，当队列为空的时候会自动触发生产者线程往里面放东西。如果用普通队列的话就会一直往队列里放东西而没有取东西，所以他会阻塞在生产者线程，不会进行线程之间的切换。

        2. 基于基于网络通讯的方式实现MQ(忽略)
            比如使用netty通信


      rockmq版本，，商业版和开源版
          RocketMQ，包含开源版本和阿里云商业版。
          RocketMQ是开源版本的mq
          云产品 Aliware MQ 则是提供了商业化的版本。

          比如
            如果您使用开源 RocketMQ，则 RocketMQ Server 和 RocketMQ Client 都需要使用 4.5.0 及以上版本。
            如果您使用阿里云 RocketMQ，需要使用铂金版，且Ons Client 使用1.8.0.Final 及以上版本。

      rockmq事务消息落地实现

      redis队列，list做简单的消息队列的功能
        使用 list Pop Push 功能可以作为阻塞队列／非阻塞队列
        使用 SUBSCRIBE PUBLISH 可以实现发布／订阅模型对数据进行实时分析，如可以累加统计等
        
        * list是一个链表结构，主要功能是push,pop,获取⼀个范围的所有的值等，操作中key理解为链表名字。
        * Redis的list类型其实就是⼀个每个⼦元素都是string类型的双向链表。我们可以通过push,pop操作从链表的头部或者尾部添加删除元素，
        * 这样list既可以作为栈，⼜可以作为队列。Redis list的实现为⼀个双向链表，即可以⽀持反向查找和遍历，更⽅便操作，不过带来了部分额外的内存开销，
        
      dubbo扩展负载均衡，比如消费端带特殊标记，服务端去自定义路由，灰度发布测试用
        nginx是应用层上层，底层负载均衡实现
      ...
      es更新同步，binlog监听
          canal对于代码没有侵入性，基于监听mysql binlog日志去实现数据同步，实时性也非常的不错
          相对于job，优点在于监听的是增量的数据，job可能全量更新(实际是全量工作中需求)

          1.mysql配置文件my.ini，开启binlog
            log-bin=mysql-bin       # 打开binlog,二进制文件存放路径
            binlog-format=ROW       # 选择ROW(行)模式
            server_id=1             # 配置MySQL replaction需要定义，不要和canal的slaveId重复(canal1.1.4自动生成slaveId)

            show variables like 'log_%'; 确认已经开启binlog
          ...
          2.创建mysql的用户
            ...
            比如，CREATE USER 'zs'@'196.168.64.12' IDENTIFIED BY '123';
            ...
            GRANT privileges ON databasename.tablename TO 'username'@'host'
              ...
              比如，GRANT SELECT,UPDATE ON test1.user TO 'ZS'@'192.169.12.12';
          ...
          3. 配置canal
              下载安装包并解压 https://github.com/alibaba/canal/releases/  ,比如canal.deployer-1.1.5-SNAPSHOT.tar.gz版本

              修改conf/canal.properties配置文件，注册地址为本机IP
                  canal.zkServers = 127.0.0.1:2181       # zookeeper集群的地址和端口
                  canal.serverMode = rocketmq            # 默认为tcp,我这里选择将监听到的消息发送到rocketMQ
                  canal.mq.servers = 127.0.0.1:9876      # mq的地址和端口，这里是rocketmq中nameserver的端口
                  canal.instance.tsdb.dbUsername = canal   # 创建的mysql slave账号和密码
                  canal.instance.tsdb.dbPassword = canal

              修改conf/example/instance.properties配置文件，配置数据库信息
                  canal.instance.filter.regex=.*\\..* ，默认监听所有数据库的所有表，也可以是具体的表名，用,隔开,比如test.t_people,test.user
                  canal.instance.filter.regex=db_xiaoyuer_20191125\\..*       指定固定数据库监听
                  canal.instance.defaultDatabaseName = db_xiaoyuer_20191125  1.1.4版本配置无效
          ...
          使用canal监听binlog，发送mq消费，实现同步操作
          可以使用java的客户端去监听，实际使用mq也行
                    <dependency>
          ...
          客户端的subscribe()方法会覆盖配置文件的配置，这个是配置不生效的元凶，但是我们不能直接不写这个方法，需要将其中条件改为空，直接为空默认会使用上一次的值
      ...
      es集群，搭建，elk那套搭建过
        es集群脑裂
          ...
          集群的定义，多个相同节点提供服务
          集群作用 1.单节点压力问题，2.数据冗余备份

          es一个也可以是集群

          ES集群由若干节点组成(cluster-name相同)
              master节点：
                任意一个节点均可以成为 master 节点
                集群中的一个节点会被选为master节点，它将负责建立或删除索引，添加节点到集群或从集群删除节点。
                master节点无需参与文档层面的变动和搜索，这个节点不会存储数据
              data节点：	
                持有数据和倒排索引。
                每一个节点可通过elasticsearch.yml中的node.data属性为true(默认)成为数据节点。
                master节点，node.data属性设置为false
              Client节点：
                若是将node.master属性和node.data属性都设置为false，那么该节点就是一个客户端节点，扮演一个负载均衡的角色，将到来的请求路由到集群中的各个节点


          分片和复制
            单节点数据放不下(一个索引包含海量文档)，就要分片，分片数据的高可用就用复制一个副本
            复制和主分片不要再同一个节点上。
            索引创建后分片数不可变，复制数可变
		        ES提供分片机制，同一个索引能够存储在不一样分片中(分片存在集群中不同节点上)

            这里是这针对索引index分片的， es自主实现分配分片和复制
              //1.索引的settings设置
              ...
              CreateIndexRequest createIndexRequest = new CreateIndexRequest(Es.INDEX_RECRUIT)
                  .settings(settings)
                  .mapping(mappings);
              
          当主分片和副本组不成一个完整索引，就要停es了，比如3挂2，复制副本只有没有对应的副本了，
          类似 redis的hash槽不完整，停redis

          查看集群状态  http://192.168.6.50:9200/_cat/health?v  

          epoch      timestamp cluster     status node.total node.data shards pri relo init unassign pending_tasks max_task_wait_time active_shards_percent
          1655169519 01:18:39  elk-cluster yellow          1         1      7   7    0    0        5             0                  -                 58.3%

          项目中每个index比如require，在节点上都有一个主分片，共7个index，所以上面的shards=7
          ...
          head插件，查看更详细的集群状态
              ...
              2.安装grunt
                  nodejs安装目录下，cmd下执行 npm install -g grunt  -cli

                  grunt -version 查看是否安装成功，会显示安装的版本号

              3. 下载head插件
　　                下载地址：https://github.com/mobz/elasticsearch-head

                    进入head文件内修改配置Gruntfile.js， 增加 hostname: ‘*‘, 属性
                    进入head/_site/app.js文件，把地址改成es所在的服务器地址，这里是本地改为localhost（默认）或者127.0.0.1
                    head安装目录下，cmd执行 npm install 
                    npm run start 启动head插件

                    http://localhost:9100/  访问head页面
            
          es集群的配置文件，elasticsearch.yml
            
              cluster.name: escluster                     #集群名称 保持一致
              node.name: node-1                           #节点名称 各节点不相同
              node.master: true                           #是否为master
              node.data: true
              path.data: /mydir/es/es7.2.0/data           #data目录
              path.logs: /mydir/es/es7.2.0/logs           #日志目录
              network.host: 0.0.0.0
              http.port: 8010                             #服务端口
              discovery.zen.minimum_master_nodes: 1       #master数量
              bootstrap.memory_lock: false
              bootstrap.system_call_filter: false
              discovery.zen.ping_timeout: 3s
              cluster.initial_master_nodes: node-1
              transport.tcp.port: 9300                    #服务间通信端口
              discovery.zen.ping.unicast.hosts: ["47.93.230.77:8012"]        #服务间互相发现地址

          容错，集群中挂一个节点，集群会自动把对应的副本恢复组成一个完整数据集
          
          es中index中的mapping主要配置字段类型和分词,定义文档中字段的存储和索引。
                es中字段的dynamic动态配置，
                  关系型数据库中，先建表，再指定字段和字段类型，才能将数据插入表中。
                  ES中，索引=表，文档=记录，文档字段=表字段，字段同样有数据类型。
                  
                  ES不需要先定义表结构，可根据写入文档的内容，自动推断字段和数据类型，创建索引结构，这就是dynamic mapping，动态映射的由来。
                  动态映射默认有一个规则，即请求中文档的什么json类型对应es中什么数值类型，也可通过动态模板（dynamic template）来覆盖这个规则，实现自定义推测规则。
                  ES自动建立mapping时, 为不同的field映射了不同的数据类型, 而不同数据类型在分词、搜索等行为上也是不同的.						

                  按dynamic值，可三种模式，默认值就是"true"
                  动态模式(true)，根据输入文档的内容，自动推断字段和类型，创建mapping
                  非动态模式(false)，无法根据输入文档的内容，自动创建mapping(需手动)，当文档中包含新的字段(不在mapping中)，依然可存储和读取，无法根据该字段进行检索，Es会忽略新的字段。
                  严格模式(strict)，索引的mapping结构不能扩展字段，当文档中包含新的字段时，Es会抛异常。无法存储，会直接报错，类似于关系型数据库中的表。
                    

                项目中实际的index-require,
                创建mapping时，
                  只指定了req_title一个字段的type=text，analyzer=ik_max_word

                添加文档时，也默认json存入
                  IndexRequest request = new IndexRequest(Es.INDEX_REQUIRE);
                  request.id(requireInfo.getId().toString());//指定id
                  request.source(JSON.parseObject(JSON.toJSONString(requireDmo)), XContentType.JSON);
                  IndexResponse response = Es.client.index(request, RequestOptions.DEFAULT);

                查案index-require-mapping，没有指定的字段都有默认属性
                ...
                Es5.0以后，移除了string类型，string字段被拆分成两种新的数据类型: text用于全文搜索的,而keyword用于关键词搜索。
                ...
                字段index属性可以取true和false两个值，默认值为true。(忽略)
                  当一个字段的index属性为true时，Es会为该字段创建索引，进而该字段可以支持搜索(精确匹配或全文搜索)。
                  当一个字段的index属性为false时，Es不会为该字段建立任何索引，也就导致该字段无法支持搜索。


          基本的概念
            文档
              es是面向文档的，文档是全部可搜索数据的最小单位, 文档会被序列化为JSON格式，保存在es中
              每一个文档都有一个Unique ID，【可本身指定或经过es自动生成】


            索引
              index ——索引，文档的容器，是一类文档的结合
              index体现了逻辑空间概念：每一个索引都有本身的Mapping定义，用于定义包含的文档的字段名和字段类型
              shard体现了物理空间的概念：索引中的数据分散在shard上
              索引的mapping与settings，前者定义文档字段的类型，后者定义不一样的数据分布
              
            节点
              节点是一个es的实例， 本质是一个JAVA进程
              一个机器可运行多个es进行，建议一个机器只运行一个
              每一个节点启动后，会分配一个UID，保存在data目录
              
              每一个节点都保存了集群状态，只有master节点可修改集群的状态信息
              集群状态信息，包括全部的节点信息、全部的索引及其相关的mapping和setting的信息、分片的路由信息

            分片
              主分片，解决数据水平扩展，可将数据分布到集群内全部节点上
              一个分片是一个运行的lucene的实例
              主分片数索引建立时指定，不准修改，除非Reindex

              索引上有海量文档时，数据可以分为较小的分片。每个分片放到不同的服务器上。
              当你查询的索引分布在多个分片上时，ES会把查询发送给每个相关的分片，并将结果组合在一起，而应用程序并不知道分片的存在。这个过程对用户来说是透明的。
            ...
            副本replica
              副本策略对index中的每一个主分片建立冗余的副本，副本数可动态调整，解决数据高可用
              副本是对应分片的备胎，当分片机器挂掉了，es能够使用其副本进行恢复，避免数据丢失，实现高可用性。
            
              需要注意，副本分片永远不会分配到与主分片相同的节点上。
              再启动一个节点将不会对主分片起到任何作用，因为主分片不可以修改，只有三个，但是副本可以修改，能够起到扩容的作用。

              
              es间的通信都是内网通讯,
              es自动killd,是因为内存不够,修改jvm.options配置即可.  
