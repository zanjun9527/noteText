...
平台提升即可，在公司的主要工作内容，技术只谈技术，杂的不要谈
尊重和平等
...
1.过期了，不执行
2.过期了，需要租约续期
3.最粗暴的方式就是将租约时间设置的时间加长，缺点是可能造成服务阻塞
...
namespace+id 作为Map<String,MappedStatement>的key使用，namespace不能重复
 ...
 主从数据库不一致问题解决
  ...
  大数据的limit优化
    ...
    核心还是找到limit m,n  的那个起始的offsetid，然后使用limit n 来查询
    1.上一页的id带过来 
...
### 1. 111
      ...
      redis主从，复制断电，数据丢失，
        注意redis集群模式中，数据是分片存储的，每个主节点存储的数据不一样，采用哈希槽的槽位映射到具体的节点，自带的哨兵功能
