...
17. 集群模式
      ...
      Broker 部署相对复杂，这里的集群模式主要指得是 Broker
...
9. @SpringBootApplication扫描范围
   ...
   @SpringBootApplication注解：标识了⼀个SpringBoot⼯程，是个组合注解：
      a. @SpringBootConfiguration：这个注解实际就是⼀个@Configuration，表示启动类也是⼀个配置类
      b. @EnableAutoConfiguration：向Spring容器导⼊了⼀个Selector，⽤来加载ClassPath下SpringFactories中所定义的⾃动配置类，将这些⾃动加载为配置Bean
