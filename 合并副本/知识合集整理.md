...
## 数据结构相关
   Collection
   ├List （有序，可重复，允许null）
   │├LinkedList   底层使用了链表数据结构，特点： 查询速度慢(要移动指针)，增删快
   │├ArrayList    底层维护了一个Object数组，特点：查询速度快，增删慢(要移动数据)
...
2. Map相关
   ...
   4. **hashmap的原理**	
      ...
      * hashmap的原理是基于hash运算计算存储位置，同一hash位置的多个entry根据key.equals（）取值   
      ...
      HashMap中put方法过程,思路很顺
         对Key求 Hash 值，然后再计算 下标。
         如果没有碰撞，直接放入桶中，
         如果碰撞了，以链表的方式链接到后面，
   ...
   5. hashmap的扩容
         ...
         扩容，将新节点加到链表后，容量扩充为原来的两倍，然后对每个节点重新计算哈希值。该值只可能在两个地方，一个是原下标的位置，另一种是在下标为 <原下标+原容量> 的位置。
...
8. 真正执行搜索的语句
      ...
      SolrDocumentList docs=rsp.getResuts() 
      ...
      redis
         Redis 支持的各种存储结构，数据类型，常用string和hash
            ...
            5. SortedSet 有序集合
                  ...
                  底层使用的结构是跳表
                     跳表为了解决插入和删除节点时造成的后续节点重新调整的问题，引入了随机层数的做法
                     每一个节点的层数都是随机算法得出的，插入一个新的节点不会影响其他节点的层数，因此，插入操作只需要修改插入节点前后的指针即可，
                     避免了对后续节点的重新调整。
...
11. Redis 主从复制、哨兵和集群三者区别
         ...
         https://juejin.cn/post/6844903872519995400
         https://blog.csdn.net/sunhuiliang85/article/details/78361211
         ...
         哨兵sentinel：高可用
            ...
            哨兵至少三台机器
               ...
               哨兵选主至少要3个节点，即majority至少2个，若哨兵集群仅部署个2个哨兵实例，宕机一个，只有一个节点，无法满足majority>=2这个条件，master故障时无法进行主从切换。
               若只有两个哨兵的话，master和sentinel1运行的整个机器如果宕机了的话，那么哨兵就只有一个了，此时就无法来通过majority来进行故障转移了，所以，我们至少需要三台哨兵实例。


            哨兵服务和redis服务是相互独立的服务, 可在不同的服务器上单独部署.
            哨兵可以跟着redis实例走，但是启动的是不同的端口，比如redis实例是，192.168.6.128:6379，对应的哨兵sentinel是，192.168.6.128:26379
            ...
            Redis Sentinel部署技巧
               ...
               3.只有一套Sentinel，还是每个主节点配置一套Sentinel的讨论的建议方案是，如果Sentinel节点集合监控的是同一个业务的多个主节点集合，那么使用方案一，否则使用方案2.
            ...
            分布式选举
               ...
               每个 Sentinel 节点会对数据节点和其余 Sentinel 节点进行监控，当它发现节点不可达时，会对节点做下线标识。
               如果被标识的是主节点，它还会和其他 Sentinel 节点进行“协商”，当大多数 Sentinel 节点都认为主节点不可达时，
               它们会选举出一个 Sentinel 节点来完成自动故障转移的工作，同时会将这个变化实时通知给 Redis 应用方。
            ...
            数据丢失问题(忽略)
               ...
               redis脑裂就是自动故障转移造成的。
                  如果是哨兵和slave同时与master断了联系，即哨兵可以监测到slave，但是监测不到master，而master虽然连接不上slave和哨兵，但是还是在正常运行，这样如果哨兵因为监测不到master，认为它挂了，会在slave中选举新的master，而有一部分应用仍然与旧的master交互。这时，集群里就会有两个master，也就是所谓的脑裂。
                  当旧的master与新的master重新建立连接，旧的master会作为一个slave挂到新的master上去，同步新的master中的数据，而旧的master中的数据会丢失。重新从新的master复制数据
         ...
         集群cluster：数据分片,分布式存储
            ...
            每个集群中至少需要三个主数据库才能正常运行，Redis集群至少需要3个主节点，每个节点又需要一个从备份节点，所以Redis集群至少需要6台服务器。
            ...
            ***数据路由***
            ...
            集群为啥3台主节点
               当一个节点挂掉后，集群超过半数的节点检查失效时该节点才会被认为挂掉。
               所以基于以上的情况，节点的数量一般都是奇数。一般为了保证安全性，每个节点由一个备份节点。所以最小的集群主节点的个数最少为3，从节点的个数也为3，共6台
            ...
            Redis集群自身实现了高可用。高可用首先需要解决集群部分失败的场景：当少数节点出现故障时，可以通过自动故障转移保证集群可以正常对外提供服务。
...
12. Redis Cluster分区方式 和 Redis扩容(忽略)
      ...
      Redis扩容(忽略)
         ...
         扩容步骤
            ...
            三、设置从节点
               cluster replicate <master_id>命令为主节点添加从节点，集群模式下不支持slaveof命令
         ...
         Redis集群扩容可以分为如下步骤：
            ...
            加入集群
               ...
               可以使用redis专门进行集群管理的工具redis-trib.rb，位于Redis的源码目录中，把6386节点加入到集群中
            ...
            迁移槽和数据
               可以使用redis-trib.rb工具，也可以通过手动命令的方式，但是一般要确保每个主节点负责的槽数是均匀的，因此要使用redis-trib.rb工具来批量完成，
...
15. **redis秒杀架构思想**
      ...
      常规方案
         页面静态化，cdn加速
            并将页面静态化之后的页面分发到 CDN 边缘节点上，起到压力分散的作用。

         防止提前下单(忽略)
            静态化页面中加入一个 JS 文件引用，包含活动是否开始的标记以及开始时的动态下单页面的 URL 参数,不会cdn缓存，
            会一直请求后端服务的，所以这个 JS 文件一定要很小。当活动快开始的时候（比如提前），通过后台接口修改这个JS文件使之生效。
         ...
         SOA 服务层优化
            后端系统的控制可以通过消息队列、异步处理、提高并发等方式解决。对于超过系统水位线的请求，直接采取「Fail-Fast」原则，拒绝掉。


      流程， 用户抢购（请求）->是否是异常用户（账号安全检查）->缓存中的库存是否足够（商品库存检查）->扔给mq去慢慢操作->异步消费订单处理，附带超时补偿操作
...
9. 杂记
   ...
   * maven依赖jar，但是没有在eclipse中导入项目就看不到源码，除非jar中有sources文件
...
Jenkins Pipeline，ansible(忽略)
	...
	ansible-playbook
      ...
      hosts文件中指定机器信息
      [pre_docker_101]
      192.168.6.101 ansible_ssh_user='pre_yunwei' ansible_ssh_port=22 ansible_ssh_pass='CKH@123$%^YunWei_pre'
      192.168.6.102 ansible_ssh_user='pre_yunwei' ansible_ssh_port=22 ansible_ssh_pass='CKH@123$%^YunWei_pre'
      指定了用户和密码
...
6. 死信队列也是一个消息队列，它⽤来存放那些没有成功消费的消息的，通常可以⽤来作为消息重试
    延时队列就是⽤来存放需要在指定时间被处理的元素的队列，通常可以⽤来处理⼀些具有过期性操
    作的业务，如几分钟内未支付则取消订单

7. 几种mq的对比	https://www.csdn.net/tags/MtTaAg4sNTc3NjAzLWJsb2cO0O0O.html

   公司业务平稳，没有巨大的并发量，也没有改源码的需求，用rabbitMQ
   公司发展迅猛，经常搞一些秒杀活动，用RocketMQ。
...
8. 系统中使用mq
      ...
      public void startConsumer() throws Exception {
         ...
         DefaultMQPushConsumer consumer = new DefaultMQPushConsumer(consumerGroup);//设置组，名称唯一
         consumer.setVipChannelEnabled(false);
         consumer.setNamesrvAddr(nameServerAddress);//设置nameserver
         // 设置Consumer第一次启动是从队列头部开始消费还是队列尾部开始消费,如果非第一次启动，那么按照上次消费的位置继续消费
         consumer.setConsumeFromWhere(ConsumeFromWhere.CONSUME_FROM_FIRST_OFFSET);
         consumer.setMessageModel(MessageModel.CLUSTERING);//设置集群消费
         consumer.subscribe(MQConstants.TOPIC_EGJ_ORDER, "*");//设置topic，订阅指定中信topic下tags不限制
...
13. 消费端 和 生产者的实例
	...
	在消费端：可以自定义listener实现源码中的messagelistenerconcurrently接口，在consumer端，监听消息的时候，可以定义一个接口(根据传参不同，选不同的service)，
...
14. mq解决数据一致性,mq解决分布式事务
      ...
      ***mq解决分布式事务的思路*****
         ...
         这里可以1.失败的模块发送多个回滚消息给各自的模块消费(集群消费，这个是不同的topic)，2.也可以发送同一个topic，各模块去消费(广播消费)
      ***mq解决分布式事务的思路*****
...
15. 生产者的可靠性投递和消费者的幂等性去重
      ...
      结构原理 https://www.bbsmax.com/A/QW5Y7GZOzm/
...
17. 集群模式
      ...
      Broker 部署相对复杂，这里的集群模式主要指的是 Broker

      RocketMQ中并没有master选举功能，在RocketMQ集群中，1台机器只能要么是Master，要么是Slave，在初始的机器配置就定死了。
      不会像kafka那样存在master动态选举的功能，所以通过配置多个master节点来保证rocketMQ的高可用。
...
18. mq的部署问题：
      ...
      windows下制作的两个启动文件
         以.bat文件中直接执行.exe文件，作为运行脚本 ，D:\alibaba-rocketmq-3.2.6\alibaba-rocketmq\bin\mqbroker.exe -n 192.168.6.222:9876 ，实际mq就是这样启动的

         mqserver.bat      D:\alibaba-rocketmq-3.2.6\alibaba-rocketmq\bin\mqnamesrv.exe
         mqbroker.bat      D:\alibaba-rocketmq-3.2.6\alibaba-rocketmq\bin\mqbroker.exe -n 192.168.6.222:9876


19. rocketmq  后台可以新增消息发送   topic选项新增消息发送
	 rocketmq常见异常 	https://blog.csdn.net/weixin_43439073/article/details/95746775

20. 为什么RocketMQ不使⽤Zookeeper作为注册中心 (忽略)
      zookeeper满足的是CP，也就是说zookeeper并不能保证服务的可用性，zookeeper在进行选举的时候，整个选举的时间太长，期间整个集群都处于不可用的状
      态，这对于一个注册中心来说肯定是不能接受的，作为服务发现来说就应该是为可用性而设计

      基于性能的考虑，NameServer本身的实现⾮常轻量，⽽且可以通过增加机器的⽅式⽔平扩展，增加集群的抗压能⼒，⽽zookeeper的写是不可扩展的，⽽zookeeper要解决这个问题只能通过划分领域，划分多个zookeeper集群来解决，首先操作起来太复杂，其次这样还是违反了CAP中的A的设计，导致服务之间是不连通的。

      持久化的机制来带的问题，ZooKeeper 的 ZAB 协议对每一个写请求，会在每个 ZooKeeper 节点上保持写一个事务日志，同时再加上定期的将内存数据镜像（Snapshot）到磁盘来保证数据的一致性和持久性，对于一个简单的服务发现的场景来说，这其实没有太大的必要，这个实现方案太重了。且本身存储的数据应该是高度定制化的。

      消息发送应该弱依赖注册中心，⽽RocketMQ的设计理念也正是基于此，生产者在第一次发送消息的时候从NameServer获取到Broker地址后缓存到本地，如果NameServer整个集群不可用，短时间内对于生 产者和消费者并不会产生太大影响。

21. mq的事务消息
      常见分布式事务解决方案有：
      1. 本地消息表：创建订单时，将减库存消息加⼊在本地事务中，一起提交到数据库存入本地消息表，然后调用库存系统，如果调用成功则修改本地消息状态为成功，如果调用库存系统失败，则由后台 定时任务从本地消息表中取出未成功的消息，重试调用库存系统
...
22. 消费模式
      ...
      例如某个Topic有九条消息，其中一个Consumer Group有三个实例（可能是3个进程，或者3台机器），那么每个实例只消费其中的3条消息，


      广播消费方式
      一条消息被多个Consumer消费，几十这些Consumer属于同一个ConsumerGroup，消息也会被ConsumerGroup中的每个Consumer消费一次，
      广播消费中ConsumerGroup概念，适用于一些分发消息的场景，比如订单下单成功了，需要通知财务系统，客服系统等等这种分发的场景，可以通过修改Consumer中的MessageModel来设置消费方式为广播消费
...
### Rabbitmq
      ...
      Spring AMQP (geekdoc.top) Spring AMQP的文档		https://geekdoc.top/docs/languages/java/spring/spring-amqp/2.3.11/reference/html/index.html
   ...
   1. 基本介绍
         ...
         重要组成部分
            ...
            Exchange：交换机，消息队列交换机，按一定的规则将消息路由转发到某个队列，对消息进行过虑。
            Queue：消息队列，存储消息的队列，消息到达队列并转发给指定的消费者，消费后会被移除队列
            Producer：
            Consumer：

   2. 消息发送接收流程
         生产者发送消息流程：
            1、生产者和Broker建立TCP连接,建立通道
            2、生产者通过通道消息发送给Broker，由Exchange将消息进行转发到指定的Queue（队列）

         消费者接收消息流程：
            1、消费者和Broker建立TCP连接,建立通道
            2、消费者监听指定的Queue(消息到达时，Broker默认将msg推送给消费者)。
            3、消费者接收到消息后ack回复,消息一旦被消费者接收，队列中的消息就会被删除


         rabbitmq的broker包含了Exchange和queue，通过bindingkey 来关联	
         生产者发送消息至指定exchange，再通过指定的routingkey匹配对应队列，
         消费者监听队列，当队列有消息时执行消费
   ...
   4. 几种消费模式
         ...
         * 基本消息模型
               ...
               queue：消息队列，可缓存消息；生产者向其中投递消息，消费者从其中取出消息。

		   * work消息模型
               producer ---> queue ---> (c1,c2)
               多了一个消费端，两个消费端共同消费同一个队列中的消息，但是一个消息只能被一个消费者获取
               可配置能者多劳模式(谁完成的快，谁多做一点)，可通过通过BasicQos方法设置prefetchCount = 1(手动ack下生效)，来调整实例消费权重
			
		   * Publish/subscribe模型(交换机类型：Fanout，也称为广播)
               p--->x--->(queue1-->c1,queue2-->c2)
               消息先发送到Exchange，再转发到队列给消费者消费


               publish/subscribe需要设置队列和交换机的绑定，
               work queue模型不需要设置，实际上work queues底层会将队列绑定到默认的交换机 ，实际建议用前者
               
               发布订阅模式允许将同一消息发送给多个消费者。使用fanout交换机(一对多，无法指定Binding Key)，会将接收到的消息路由到每一个跟其绑定的queue(队列)
               与binding-key和routing-key无关，将接受到的消息分发给有绑定关系的所有队列
         ...
         * Routing 路由模型(交换机类型：direct)
               p--->x--->(queue1-->c1,queue2-->c2)，这里指定了routing key的消费
               ...
               使用的是Direct类型的交换机，会将接收到的消息根据规则(Routingkey)路由到指定的Queue(队列)，因此称为路由模式
         ...
         * Topics 通配符模式（交换机类型：topics）
            ...
            绑定队列和交换机关系时，支持模糊匹配，比如：user.* 和 user.#，通配符专用(标准化)，(尽量不要用单个全词，会有适配不准问题)
               routing key为一个句点号 " . " 分隔的字符串，如"One.Two"
            ...
            用消息的routing-key 与 exchange和queue间的binding-key 进行模式匹配，当满足规则时，分发到满足规则的所有队列
            
         * RPC模型示(忽略)

   5. Exchange交换机相关
         Exchange交换机：接收生产者发送的消息，处理消息(递交给某个特别队列、递交给所有队列、或是将消息丢弃)
         ...
         并通过binding-key 与 routing-key 的匹配关系来决定将消息分发到指定queue
         ...
         Exchange类型有以下几种：
            ...
            Topic：通配符，
                  ...
                  与direct模型相比，多了个可以使用通配符，这种模型Routingkey一般都是由一个或多个单词组成，多个单词之间以"."分割，
                  如：item.insert ---星号 匹配一个1词 , 例audit.*
                                    --- #号匹配一个或多个词 audit.#
            Header(忽略)：
                  header模式与routing不同的地方在于，header模式取消routingkey，使用header中的 key/value（键值对）匹配队列。
                  headers类型的Exchange不依赖于routing key与 binding key的匹配规则来路由消息，而是根据发送的消息内容中的headers 属性进行匹配。（headers 类型的交换器性能差，不实用，基本不会使用。）
            x-delayed-message(忽略)	
                  延迟交换机，可以延迟接收消息
         ...
         public Binding geteBinding(TopicExchange topicExchange,Queue queue){
            return BindingBuilder.bind(queue).to(topicExchange).with("ERRORRF");
         }

   6. rabbitmq后台页面，管理页面
         还可以查看队里中的消息，queue中的getmessage，也可以手动publish消息

         rabbitmq和erl的版本要对应上，https://www.rabbitmq.com/which-erlang.html
         erl地址，https://www.erlang.org/patches/otp-24.0
         mq地址，https://www.rabbitmq.com/
         

         Erlang (SMP,ASYNC_THREADS) (BEAM) emulator version 9.3  这个是安装的20版本的
         如果你选择了RabbitMAQ 3.7.7+以上的版本，那么请选择Erlang 20.3或21.0的版本
         
         安装了rabbitmq的地址后，后台地址，http://localhost:15672

         rabbitmq的界面上看到的消息会是乱码(忽略)
            <dependency>
   ...
   7. 相关参数
         * Virtual Hosts，虚拟主机
            初始化连接，java.net.ConnectException: Connection refused: connect
            报错原因，access to vhost '/' refused for user ，需要在后台配置用户的vhost权限
            
            为了让各个用户可以互不干扰的工作，RabbitMQ 添加了虚拟主机（Virtual Hosts）的概念
            其实就是一个独立的访问路径，不同用户使用不同路径，各自有自己的队列、交换机，互相不会影响对方
            可以再后台页面配置
         
            每个Rabbit都能创建很多vhost，每个虚拟主机其实都是mini版的RabbitMQ，拥有自己的队列，交换器和绑定，拥有自己的权限机制。
            不同用户使用同一个 RabbitMQ server 提供的服务时，可以划分出多个 vhost，每个用户在自己 vhost创建 exchangqueue 等(类似RocketMQ中的Group)。
            
            RabbitMq相当于MySQL，RabbitMq中的VirtualHost就相当于MySQL中的一个库
         ...
         * Channel
					...
					Channel 作为轻量级的Connection 极大减少了操作系统建立 TCP connection 的开销df
         ...
         * BindingKey（绑定键）
					用于把交换器的消息绑定到队列上；
					是在配置时指定交换机和队列的绑定路由键，是为了去匹配生产者发送消息指定的路由键；
					每个交换机和队列之间都会有一个对应的绑定路由，首先消息发送到指定交换机，再根据发送的路由规则匹配事先设置的绑定路由键，匹配到对应的绑定路由则代表消息找到对应的队列。
               ...
               自定义的要作为消息object发送的类一定要实现Serializable接口，一般也不用
      ...
      * 死信队列
            ...
            QueueBuilder.deadLetterExchange("Dead_Exchange")		//设置死信交换机参数
			...
			代码实例
				...
				public Queue directQueue(){
					...
					args.put("x-dead-letter-exchange", "dead_exchange");  //声明当前队列绑定的死信交换机
					args.put("x-dead-letter-routing-key", "dead");        //声明当前队列的死信路由 key
					//1.队列名 2.是否持久化 3.是否独占 4.自动删除 5.其他参数
   ...
   8. 简单的测试代码
            //这里使用java代码客户端amqp方式，使用的是5672端口，15672是http的方式
            //Connection是publisher／consumer 和 broker 之间的 TCP 连接，它封装了socket协议相关部分逻辑。ConnectionFactory为Connection的制造工厂

				public class RedissonTest {
					private static Logger logger = LoggerFactory.getLogger(NetpayTest.class);
					private static Connection connection = null;
					static {
						//创建Connection工厂
						ConnectionFactory factory = new ConnectionFactory();
						factory.setUsername("zhanjun");
					...
					//消费者
					public static void publishListen() throws Exception {
						//2、创建channel
						Channel channel = connection.createChannel();
						//3、创建队列-helloworld
						/**
				...
				一般来讲，消费者更关注的是自己的 queue，包括定义 queue 并与 exchange 绑定，而这一套过程是可以直接通过rabbitmq的控制台操作的哦
				
				case1: exchange, queue 已存在
				   实际开发过程中，exchange 和 queue 以及对应的绑定关系已经存在的可能性是很高的，并不需要再代码中额外处理，直接指定注解中的queues参数即可，参数值为对列名(queueName)
		
				case2: queue 不存在
				   当queue 的 autoDelete 属性为 false 时，上面的使用场景还是比较合适了；但是，当这个属性为 true 时，没有消费者队列就会自动删除了，这个时候再用上面的姿势，可能会异常
   ...
   9. boot整合rabbitmq 
         //属性文件配置
            ...
            spring.rabbitmq.username=zhanjun
         ...
         //mq配置代码
            ...
            public class RedissonTest2 {
                  ...
                  //3、绑定，这里的topicExchange和queue可以直接获取到上面注入的bean实例，也可以自己调方法获取
                  @Bean
                  public Binding geteBinding(TopicExchange topicExchange,Queue queue){
                     return BindingBuilder.bind(queue).to(topicExchange).with("ERRORE");
                  }
            }

         //配置消费者
            @Component
            public class GTestcc {
                  @RabbitListener(queues = "topic-queue")
                  public void getMassage(Object massage){
                     Message obj = (Message)massage;
                     System.out.println("接收到消息："+new String(obj.getBody()));
                  }
            }
         ...
         //生产者发送消息
            ...
            @RequestMapping("ggg")
            public void gggg(HttpServletRequest request, HttpServletResponse response) {
               System.out.println("ceshilaisl");
               rabbitTemplate.convertAndSend("boot-topic-exchange","ERRORE","就很搞笑.go");
               rabbitTemplate.convertAndSend("boot-topic-exchange","ERRORE.nis","就很搞笑.ni");
            }

            
   9. Message acknowledgment（消息确认）
			从安全角度考虑，网络是不可靠的，接收消息的应用也有可能在处理消息的时候失败。
			基于此原因，AMQP模块包含了一个消息确认（message acknowledgements）的概念：当消息从队列投递给消费者的时候，消费者服务器需要返回一个ack(确认信息)，当broker收到了确认才会将该消息删除；消息确认可以是自动的，也可以是由消费端手动确认。
			此外也支持生产端向broker发送消息得到broker的ack，从而针对做出响应逻辑。
   ...
   11. 备用交换机
				...
				public class RabbitDirectConfig {
					...
					public DirectExchange directExchange(){
						...
						//1.交换器名 2.是否持久化 3.自动删除 4.其他参数
						Map<String,Object> args = new HashMap<>(3);
						args.put("alternate-exchange","Alternate_Exchange");//替补交换机
...
1. 两种动态代理的模式：   
         ...
         3.javassist(忽略)		
               javassist 字节码生成，提供的动态代理工场，生成代理类，使用MethodHandler接口 ，使用java反射性能比较低，主要是在反射上面。自己生成代理类 直接调用
...
5. 两种事务管理方式：
   ...
   * 1.编程式事务管理
     ...
     使用 TransactionTemplate(常用)
      ...
      直接使用PlatformTransactionManager(忽略,古董写法)
...
6. 如何在spring事务提交之后进行一些操作，这些操作必须得在该事务成功提交后才执行，回滚则不执行。
      如何在spring事务提交之后操作
      如何在spring事务回滚之后操作
      实现方案
      ...
      复制代码
      if(intApplyIds != null && intApplyIds.size()>0) {
            String strApplyIds = listToString(intApplyIds);
            TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronizationAdapter() {
               @Override
               public void afterCommit() {
                  asyncOpenAutoDepositPayService.asyncDepositPay(strApplyIds, ri.getUserId());//异步，选人、托管支付回调
                  super.afterCommit();
               }
            });
      }
...
8. **非事务声明方法调用事务声明方法，则事务失效**，事务自调用失败
   ...
   sping的事务不能回滚
      ...
      事务方法内部捕捉了异常，没有抛出新的异常，导致事务操作不会进行回滚(常见忽略)
      ...
      对于静态static方法 和 public 方法 注解@Transactional会失效
            ...
            两个方法都被@Transactional注解标注，但是一个有public修饰符一个没有，那么这种情况我们可以预见的话，一定会创建代理对象，因为至少有一个public修饰符的@Transactional注解标注方法。

            创建了代理对象，非public 方法insertTestWrongModifier就也不会开启事务吗,不执行代理@Transactional对应的代理逻辑
            因为，代理过程中目的是获取当前被代理对象的当前需要执行的method适配的aop逻辑。
...
9. @Transactional
   ...
   因为底层cglib是基于父子类来实现的，子类是不能重载父类的private方法的，所以无法很好的利用代理，也会导致@Transactianal失效
	   ...
	   * 隔离级别
         mysql默认的级别是可重复读(只有幻读的可能)。
			...
			可重复读:一个事务在执行过程中可以看到其他事务已经提交的新插入的记录，但是不能看到其他事务对已有记录的更新 
                  ...
                  在读取数据的瞬间必须先对其加共享锁 ，但是直到事务结束才释放 ，保证了可重复读（既是其他的事务职能读取该数据，但是不能更新该数据）
...
2. BeanFactory 是 Spring 框架的基础设施，面向 Spring 本身；
   ApplicationContext 面向使用Spring 框架的开发者，几乎所有的应用场合我们都直接使用 ApplicationContext 而非底层的 BeanFactory。

   对于单实例(singleton)Bean来说，BeanFactory 会缓存 Bean 实例，第二次使用 getBean() 获取 Bean 时将直接从IoC 容器的缓存中获取 Bean 实例。
   缓存池,它是一个用 HashMap 实现的缓存器，单实例的 Bean 以beanName 为键保存在这个 HashMap 中

   但是ApplicationContext除开继承了BeanFactory之外，还继承了MessageSource、ApplicationEventPublisher等接⼝，从⽽
...
3. 配合视图解析器会用，配置欢迎页，在web.xml中的欢迎页中也可以使用
   ...
   ```
   tomcat欢迎页
      如果只有一个welcome-file时，直接将welcome-file的值拼接到根目录后面进行匹配
      ...
      tomcat 默认配置了默认的index页面
      将webapp下的index.jsp去掉，使用别的名字即可
...
4. Spring MVC 中的Servlet加载
   ...
   Servlet Servlet 容器在启动时自动创建 Servlet，这是由在 web.xml 文件中为 Servlet 设置的<load-on-startup>属性决定的。
   从中我们也能看到同一个类型的 Servlet 对象在 Servlet容器中以单例的形式存在。
...
5. mvc和spring的父子容器
      ...
      * spring中的子父容器
         ...
         ServletContext
            ...
            整个 Web 应用上下文对象将作为属性放置到 ServletContext中，以便 Web 应用环境可以访问 Spring 应用上下文。
            WebApplicationContext可以通过getServletContext()获得ServletContext 的引用，
         ...
         Spring的IoC 容器可以建立父子层级关联的容器体系，子容器可以访问父容器中的 Bean，但父容器不能访问子容器的 Bean。
      ...
      * 为什么controller不直接交给spring 管理而要spring MVC容器单独分开来管理
         Spring容器与SpringMVC容器是父子关系，子容器可以访问父容器的对象，父容器不能访问子容器的类，
         如果我们在springMVC配置文件中，直接扫描所有类，把service、dao、controller都交给SpringMVC去管理是可以的，但是假如用spring去管理Contoller，它是访问不到这个类的，
         因为contoller是在springMVC容器中,假如在Spring配置直接扫描所有类包括controller，而不配置springMVC的话，这样服务器发来的请求，将会出现404的问题，因为它找不到controller，spring无法注入Controller

         spring无法为springmvc创建controller对象，所以不能吧controller交给spring管理，父子容器的关系
...
6. 获取mvc中requestmapping对应的所有的url

      可实现功能：  未登录情况下，访问.htm的404路径，应该直接跳首页，而不是先走filter跳转登录后，然后跳404

         controller
            1. 未登录，带htm，路径访问不带，404
            2. 未登录，不带htm，路径访问带，首页		
            3.登录后，https://www.xiaoyuer.net/ids/unregister/validate11.htm   404

      全部功能
         RequestMappingHandlerMapping bean = SpringContextUtil.getApplicationContext().getBean(RequestMappingHandlerMapping.class);
         Map<RequestMappingInfo, HandlerMethod> handlerMethods = bean.getHandlerMethods();
         Set<RequestMappingInfo> keySet = handlerMethods.keySet();
         List<Map<String, String>> list = new ArrayList<Map<String, String>>();
         
         public static Set<String> mvcUrls = new HashSet<String>();

         for (Entry<RequestMappingInfo, HandlerMethod> m : handlerMethods.entrySet()) {
      ...
      // RequestMappingInfo 相关格式
      [{ /manual/changePhone/apply}, {GET /findPwd/toResetPwdByEmail}, {POST /findPwd/checkCellPhoneValidateCode}, 
      {POST /findPwd/genCellPhoneCheckCode, produces [application/json]}, { /error}, { /error, produces [text/html]}]
...
9. 重定向和跳转相关，重定向相关
   ...
   redirect 和 forward 区别
      ...
      请求转发是服务器行为，一次请求，地址不变，访问自身web资源，传输数据不会丢失，会附带request中的data
      重定向是客户端行为，送到client端再一次request,两次请求，浏览器地址变化，可访问自己web外的资源，传输数据会丢失

      经常使用的一种技术叫做URL重写，就是把 session id 直接附加在 URL 路径的后面。
...
14. post 和 get 的区别
      method=”post”:这是传递大量数据时用的，传递之前会先将数据打包，数据传输是不可见的，抓包可在HTTP包的包体中看到,kv放在请求体中
      method=”get”：以URL传递的，地址栏长度有限，对数据量是有限制的，数据传输是可见的,追加kv在url中
...
24. 跨域相关		
      ...
      如果是img、iframe、script等标签的src属性去访问则是可以的，但是ajax不能
      ...
      解决方法：对于3和4，重点是使用同一域名
         1、响应头添加Header允许访问											类似文件 CorsFilter，后端filter中设置设置res.setHeader("Access-Control-Allow-Origin", "*");
            resp.setHeader("Access-Control-Allow-Origin", "*");表示可以访问所有网站，不受是否同源的限制



         2、jsonp 只支持get请求不支持post请求
            基于script标签来实现的，因为script标签是可以跨域的

         3、httpClient内部转发，转发到后端去请求资源（用过）
            后台自己控制，先访问同域名下的接口，然后在接口中再去使用HTTPClient等⼯具去调用目标接口
      ...
      jsonp原理(忽略)
         动态添加一个script标签，使用该标签的src属性没有跨域的限制的特点实现跨域。
...
1. sql中的${} 和#{} sql注入
   ...
   * #{}: 是以预编译的形式，将参数设置到SQL语句中;PreparedStatement:防止SQL注入，将传入的数据都当成一个字符串处理;
             预编译处理,将sql中的#{}替换为？号，并调用PreparedStatement的set赋值
...
18. sql的组成部分和执行过程
      ...
      三.statement 每次执行sql语句，相关数据库都要执行 sql 语句的编译，preparedstatement 是预编译得,preparedstatement支持批处理(相同句法的SQL语句，只是变量不同)
...
4. boot中的web配置
   ...
   主要是配置拦截器，过滤器，servlet，静态资源等一些web配置
   ...
   配置错误页面
      @Configuration
      public class ServletContextConfig implements ErrorPageRegistrar,WebMvcConfigurer{
         
         @Autowired
         private HandlerInterceptorCommon handlerInterceptorCommon;
         ...
         public void registerErrorPages(ErrorPageRegistry registry) {
            registry.addErrorPages(
                  new ErrorPage(HttpStatus.BAD_REQUEST, "/error/404"),//400
                  new ErrorPage(HttpStatus.NOT_FOUND, "/error/404"),//404
                  new ErrorPage(HttpStatus.FORBIDDEN, "/error/runtimeException"),//403
                  new ErrorPage(HttpStatus.INTERNAL_SERVER_ERROR, "/error/runtimeException")//500
                  );
         }
      }
...
9. @SpringBootApplication扫描范围
   ...
   @SpringBootApplication注解：标识SpringBoot启动类入口，是个组合注解：
      a. @SpringBootConfiguration：这个注解实际就是一个@Configuration，表示启动类也是一个配置类
      b. @EnableAutoConfiguration：向Spring容器导入了一个Selector，用来加载ClassPath下SpringFactories中所定义的自动配置类，将这些自动加载为配置Bean
...
14. 创建boot工程 
      1.http://start.spring.io/  快速创建工程   官网的模板下载
            创建好了之后，再导入eclipse中(可能要复制一个.project文件，并更新下项目名)

      2.自定义banner的地址：http://patorjk.com/software/taag
      3.eclipse中添加一个插件sts(Spring Tools Suite)，其中使用spring stater project创建工程boot工程
            目前eclipse一般都用4版本以上的sts支持boot,实测eclipse 2021-09 ok

      4.单纯的建立maven工程，添加boot的parent依赖，添加相关的简化配置
...
16. springboot 注册 servlet、filter、listener
      ...
      interceptor：实现WebMvcConfigurerAdapter，或者WebMvcConfigurationSupport可以注册interceptor，
               ...
               protected void addInterceptors(InterceptorRegistry registry) {
                  registry.addInterceptor(new UsercenterInterceptor())
                  .addPathPatterns("/usercenter/setting/*")
                  .addPathPatterns("/usercenterent/setting/*");//.excludePathPatterns(FAVICON_URL);
...
22. 多语言
      1. resource目录下放置messages_zh_TW.properties等文件，并且加入容器扫描

      2. 核心的方法(单个字段获取的方法)
         @Autowired
         MessageSource messageSource;
         messageSource.getMessage(pvsKey, null, LocaleContextHolder.getLocale());
      ...
      3. 拦截器
         ...
         public ReloadableResourceBundleMessageSource messageSource() {
            ...
            //指定资源文件位置，也可以application.properties中配置,spring.messages.basename=i18n.message,
            //注意这里既代表了目录结构(/resources/i18n/message_zh_CN.properties)，也代表了前缀，message_zh_CN.properties
            messageSource.setBasename("classpath:config/messages");			
            messageSource.setCacheSeconds(3000);
            return messageSource;
         }

         //CookieLocaleResolver通过浏览器的cookie设置和取得Locale对象。
         //CookieLocaleResolver 就是将请求中的lang存在cookie中，
         //localeCookie=zh-TW	
         ...
         //如果应用程序已经使用会话那么SessionLocaleResolver是一个合理的选择。但是，如果需要Locale选择保持时间超过会话的生命周期，那么CookieLocaleResolver是唯一的选择。这里没有明显的赢家，所以你应该选择最适合情形的选项。
         //http://192.168.6.222:8150/ids/testgo?lang=zh_CN
         @Bean
         public LocaleResolver localeResolver() {
            CookieLocaleResolver localeResolver = new CookieLocaleResolver();
            //localeResolver.setCookieName("localeCookie");
            //localeResolver.setCookieMaxAge(3600);//设置cookie有效期,暂时没看到用处
            //设置默认区域
         ...
         //加入到配置文件中，实现WebMvcConfigurer接口
         @Override
         public void addInterceptors(InterceptorRegistry registry) {
            //registry.addInterceptor(handlerInterceptorCommon).addPathPatterns("/**");
            registry.addInterceptor(localeChangeInterceptor());
         }
         
         /**
            * 直接配置@Configuration，用的少
         ...
         //这里其实就是类似上面的拦截器localeChangeInterceptor和localeResolver，功能类似
         //这里没法做cookie的lang缓存，使用的是默认，localeChangeInterceptor方法可以使用缓存
         protected static class NativeLocaleResolver implements LocaleResolver {
            ...
            public Locale resolveLocale(HttpServletRequest request) {
                  ...
                  System.out.println("lang is: " + language+"-----url:"+requestURI);
      ...
      4. 定义获取类，读取工具类
         不封装就不用这配置了(直接使用messageSource.getMessage(pvsKey, null, LocaleContextHolder.getLocale());即可)，封装，就配置下
         @Component
      ...
      5. 页面配置
         ...
         //这里感觉像是类似上面拦截器的效果,这里是自定义的LocaleResolver
         protected static class NativeLocaleResolver implements LocaleResolver {
            @Override
            public Locale resolveLocale(HttpServletRequest request) {
                  String language = request.getParameter("lang");
         ...
         不同的前端方式，取属性值的方法也不一样
         https://blog.csdn.net/weixin_39738667/article/details/110509950		不同前端页面的实现方式	
         jsp取值格式
         <%@ taglib prefix="spring" uri="http://www.springframework.org/tags" %>
         <spring:message code="userName"/>
         
         messages.properties文件一定要有(内容可以为空)，否则报错No message found under code 'messageName' for locale 'zh_TW'.
         这里测试，在属性文件中配置是这样要求的，spring.messages.basename=i18n.message
         但是代码配置，没有要求
         @Bean
         public ReloadableResourceBundleMessageSource messageSource() {
            ReloadableResourceBundleMessageSource messageSource = new ReloadableResourceBundleMessageSource();
         ...
         正常页面请求，lang is: zh_CN-----url:/ids/testgo
         页面标签请求，lang is: zh_CN-----url:/ids/jsp/registerandlogin/test.jsp
            
         SpringBoot默认国际化文件为：classpath:message.properties，如果放在其它文件夹中，则需要在application.properties配置属性spring.messages.basename：

         <a href="?lang=ft_CN">繁体中文</a><br>
         https://192.168.6.222:8150/ids/testgo?lang=ft_CN
         
         单个值的转换
         https://blog.csdn.net/yb223731/article/details/85252334

         Locale locale = LocaleContextHolder.getLocale();			初始默认  en(Locale.ENGLISH)
         localeResolver.setDefaultLocale(Locale.ENGLISH);
         ...
         Locale locale = Locale.getDefault();
         logger.info("默认语言："+locale.toLanguageTag());       
         locale=Locale.SIMPLIFIED_CHINESE;       
         logger.info("更新后默认语言："+locale.toLanguageTag());

      
         <li class="nav_t-li fl">
            <a href="?lang=zh_CN">&nbsp;&nbsp;&nbsp;&nbsp;简体中文</a> &nbsp;&nbsp;
            <a href="${serverAddrs}${cpath}/index?lang=zh_TW">繁体中文</a><br>
         </li>


         就是存在就第一匹配，不存在，优先后面的，classpath:config/messages_zh_CN，自动匹配
         传en的话，默认集合，[classpath:config/messages_en, classpath:config/messages_zh_CN, classpath:config/messages_zh, classpath:config/messages]
         传ft_CN,集合变成了，[classpath:config/messages_ft_CN, classpath:config/messages_ft, classpath:config/messages_zh_CN, classpath:config/messages_zh, classpath:config/messages]
         传zh_CN话,默认集合，[classpath:config/messages_zh_CN, classpath:config/messages_zh, classpath:config/messages]
         ...
         * 根据ip选择语言
            ...
            if(location == null) {
               area.setCountry("开放接口:请更新离线数据包");
            }else {
               try {area.setCountry(location.split("\\|")[0]);} catch (Exception e) {}
               try {area.setArea(location.split("\\|")[1]);} catch (Exception e) {}
               try {area.setRegion(location.split("\\|")[2]);} catch (Exception e) {}
               try {area.setCity(location.split("\\|")[3]);} catch (Exception e) {}
               try {area.setIsp(location.split("\\|")[4]);} catch (Exception e) {}
            }

            public class ClinetIp {
               private static Logger LOG = LoggerFactory.getLogger(ClinetIp.class);
               public static String getIpAddr(HttpServletRequest request) {
                  String ip = request.getHeader("X-Forwarded-For");
                  if(ip!=null&&!"".equals(ip)){
                     String[] ipList = ip.split(",");
                     for(int i=0;i<ipList.length;i++){
                        if("unknown".equals(ipList[i])){
                           ip = ipList[i];
                           break;
                        }
                     }
               ...
               /**
               * 根据ip获取location，返回字符串格式:"国家|大区|省份|城市|运营商",若查询失败则返回null，各站的离线数据包请经常维护更新<br>
               ...
               */
               public static String getIp2Region(String ip) {
                  ...
                  try {
                     ...
                     String dbfile = ClinetIp.class.getResource("/config/ip2region.db").getPath();
            ...
            * 多环境不走后台，直接index.jsp访问的话，
               会有Caused by: javax.servlet.ServletException: javax.servlet.jsp.JspTagException: No message found under code 'top_req_introduction' for locale 'en_US'.
               主站是因为web.xml中配置了欢迎页，目前限定首页繁体路径

            * 放在jsp中可以直接读取变量的值，但是放在单独的js文件中，jsp引入该js文件，这时候识别不了
               在js文件中是无法获得session中的数值的，只可以在jsp页面中用正则表达式获得。
               产生一个xml名称的变量，这个变量在js文件中可以直接调用。如果在js文件中对又声明了一个相同名称的变量，则从jsp页面中获取的变量将会背覆盖。
                  <script type='text/javascript'>
                     var xml = "${requestScope.xml}";
                  </script> 
                  
               <spring:message code="top_Finance"/>
               var top_pre_appy = "<spring:message code='top_pre_appy'/>";  
               "/usercenter/message/SignupNotice/part.htm" >' + top_pre_appy + top_pre_appya + '   
               "/usercenter/message/SignupNotice/part.htm" >top_pre_appy"
               '<li class="clearFix"><a  class="color-333 font-14 fl" href="/usercenter/message/SignupNotice/part.htm" >' + top_pre_appy + top_pre_appya + '</a>{{if signupnotread!=0}} <i class="color-red fr">{{signupnotread}}</i>{{/if}}</li>',
...
### Filter 过滤器
   ...
   2. Filter只是链式处理，请求依然放行到目的地址
      ...
      public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
            ...
            Object userInfo = httpRequest.getSession().getAttribute(Constants.USER_LOGIN_SESSIONKEY);
            if(userInfo != null){
               chain.doFilter(request, response);
            }else{			
            
               //初始化mvcUrls，移到BaseInfoInitServlet中，启动加载     ,/ids/aka.htm
               if(!BaseInfoInitServlet.mvcUrls.contains(httpRequest.getServletPath())) {
                  logger.warn("用户访问不存在的路径servletPath：{},",httpRequest.getServletPath());
                  httpResponse.sendRedirect(Constants.URL_WEB_HOST);
                  return;
               }
...
6. 拦截器中设置值
      public class UrlActivityInterceptor extends HandlerInterceptorAdapter
      {
         @Autowired
         private IXyeVersionService xyeVersionService;
         public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler,ModelAndView modelAndView) throws Exception
         {
            // 判断请求类型
            boolean isHttps = HttpServletRequestUtil.isHttps(request);
            // 获得静态资源dto
            XyeStaticDto xyeStaticDto = xyeVersionService.getXyeStaticDto(isHttps);
            request.setAttribute("domain", xyeStaticDto.getDomain());
            request.setAttribute("ctx", xyeStaticDto.getCtx());
            request.setAttribute("xyeVersion", xyeStaticDto.getXyeVersion());
            request.setAttribute("gyUrl", xyeStaticDto.getGyUrl());
            request.setAttribute("bbsUrl", xyeStaticDto.getBbsUrl());
         }
      }
...
1. 单例模式	,延迟加载，懒加载
   ...
   *****确保一个类只有一个实例，并且提供一个全局访问点。*****分别对应构造私有和静态访问
   ...
   单例模式的生命周期
      一般单例模式创建的对象是由类的static变量引用着的，JVM如果采用可达性分析算法来回收的话，该对象是永远不可能被回收的（从创建以后）。
   ...
   几种实现模式
      ...
      * 懒汉模式
            ...
            注意一个实例化同步问题，避免创建了两个实例。这里直接将synchronized放在getInstance上也行
            if (instance == null){
...
4. 装饰者模式（Decorator）(忽略)
      ...
      ServletRequestWrapper和HttpServletRequestWrapper 提供对 request 对象进行包装的方法，但是默认情况下每个方法都是调用原来 request 对象的方法，也就是说包装类并没有对request 进行增强
      在这两个包装类基础上，继承 HttpServletRequestWrapper ，覆盖需要增强的方法即可
      request 对象的增强
      1.Servlet API 中提供了一个request对象的Decorator设计模式的默认实现类HttpServletRequestWrapper ,(HttpServletRequestWrapper类实现了request接口中的所有方法，
      但这些方法的内部实现都是仅仅调用了一下所包装的的 request 对象的对应方法),以避免用户在对 request 对象进行增强时需要实现 request 接口中的所有方法。
      ...
      装饰者模式和代理模式。
         代理模式是行为的扩展，装饰者模式是对象的扩展；两者的区别有点类似于接口和继承的区别。

      ​   说明代理模式和装饰者模式的区别–代理模式偏向于增强行为动作，装饰者模式偏向于增强类的能力
         然后说明代理模式的使用场景Spring AOP就是用代理模式实现的，装饰者模式的使用场景InputStream的增强装饰类就是FileInputStream。
...
### 基本知识点
   CAP定理
      ...
      C（Consistency）：数据一致性。
         分布式系统中，数据会有副本。由于网络或者机器故障等因素，可能有些副本数据写入正确，有些却写入错误或者失败，这样就导致了数据的不一致了。而满足数据一致性规则，就是保证所有数据都要同步。

        客户端知道一系列的操作都会同时发生(生效)，数据一致更新，所有数据变动都是同步的。最终一致即可。
        在数据同步过程中，是不能对外提供服务的，不然就会造成数据不一致，所以强一致性和可用性是不能同时满足的


      A（Availability）：可用性。
         我们需要获取什么数据时，都能够正常的获取到想要的数据（当然，允许可接受范围内的网络延迟），也就是说，要保证任何时候请求数据都能够正常响应。
         每个操作都必须以可预期的响应结束，分布式系统对外要保证可⽤。

      P（Partition Tolerance）：分区容错性。
         当网络通信发生故障时，集群仍然可用，不会因为某个节点挂了或者存在问题，而影响整个系统的正常运作。
         分布式系统内即使出现单个组件节点无法可用,操作依然可以完成

         实际上分区容错性指的是分区中2个服务之间通信，如果有一个停了，另一个还需要对外提供服务，只是表现为数据缺失或者怎么样，不能因为一个或两个服务的错误导致整个系统瘫痪
      ...
      CAP 定理，仅仅针对于分布式系统，
         ...
         一个分布式系统里面，节点组成的网络本来应是连通的。可因为一些故障，使有些节点之间不连通了，整个网络就分成了几块区域。数据就散布在了这些不连通的区域中。这就叫分区。
      ...
      zookeeper实现了cap中的cp特性，因为zk集群有leader选举机制，牺牲了短暂的可用性，但是保证了容错性(CP - Redis)
   ...
   BASE理论
      ...
      最终一致性，不要求分布式系统数据实时达到⼀致，允许在经过一段时间后再达到一致，在达到一致过程中，系统也是可用的


   https://www.cnblogs.com/soundcode/p/5590710.html			分布式一致性
...
* 9. 其他
      ...
      分布式id要考虑两个点 1.唯一性  2.连续性
         1. uuid                   简单，不连续
         2. redis生成id            用Redis的原子操作 INCR和INCRBY

               可以使用Redis集群来获取更高的吞吐量。假如一个集群中有5台Redis。可以初始化每台Redis的值分别是1,2,3,4,5，然后步长都是5。各个Redis生成的ID为：
               A：1,6,11,16,21 B：2,7,12,17,22 C：3,8,13,18,23 D：4,9,14,19,24 E：5,10,15,20,25
               这个，随便负载到哪个机确定好，未来很难做修改。但是3-5台服务器基本能够满足器上，都可以获得不同的ID。但是步长和初始值一定需要事先需定好。
      ...
      存储拆分后如何解决唯一主键问题
         ...
         redis，mongodb，zk等中间件：增加了系统的复杂度和稳定性，⽐如redis的自增命令、zookeeper的顺序节点
         雪花算法
            利⽤雪花算法也可以生成分布式ID，底层原理就是通过某台机器在某一毫秒内对某一个数字自增，这种方案也能保证分布式架构中
...
4. sleep和wait的区别
      ...
      wait(),线程会放弃对象锁,进入线程等待池中，让出系统资源。需线程调用notify/notifyAll方法唤醒(并不立刻释放锁，执行完锁代码块才释放)，才进入就绪队列中等待系统分配资源,重新参与获得锁的竞争
...
5. wait(),notify(),notifyall()
   ...
   * 线程执行wait()后，就放弃了运行资格，处于冻结状态；
   ...
   同一把lock的wait 和notify要对应起来使用(wait和notify方法的调用一定要是同一个对象)
   ...
   调用wait()方法的时候，线程会放弃对象锁，进入等待此对象的等待锁定池，只有针对此对象调用 notify()方法后本线程才进入对象锁定池准备获取对象锁进入运行状态。
   调用notify()方法，唤醒在此对象监视器上等待的单个线程，如果所有线程都在此对象上等待，则会选择唤醒其中一个线程，选择是任意的
...
6. 常用方法
   ...
   ​	    join在当前的线程中加入该线程，父线程等待子线程结束后才能继续运行。
         ...
         ***** 注意要先threada.start() 后 threada.join()才能看到效果 *****
	...
	* interrupt() 忽略
         ...
         作用：给线程设置中断状态，只设置状态，不会立刻停止，后续可检查自己的中断状态isInterrupted(),根据中断状态做一些自定义的处理
         ...
         Thread.interrupt 的作用其实也不是中断线程，而是通知线程应该中断了，具体到底中断还是继续运行，应该由被通知的线程自己处理。
         调用 interrupt方法是在当前线程中打了一个停止标志，并不是真的停止线程。

         当我们调用th1.interrput()的时候，线程th1的中断状态(interrupted status)会被置位。我们可以通过Thread.currentThread().isInterrupted() 来检查这个布尔型的中断状态
         

         判断线程是否停止状态，Thread.java 类中提供了两种方法：
         ...
         2. 若线程处于正常活动状态，会将该线程的中断标志设置为true，仅此而已。被设置中断标志的线程继续正常运行，不受影响。interrupt()并不能真正中断线程，需要被调用的线程自己进行配合才行。
         ...
         中断一个Java线程，可调用线程类（Thread对象interrupt()方法(并不会立即执行中断操作,只是给线程设置一个为true的中断标志（布尔变量）)；
         设置之后根据线程当前的状态进行不同的后续操作。

         如果，线程的当前状态处于非阻塞状态，那么仅仅是线程的中断标志被修改为true而已；在此基础上，一旦进入阻塞状态，则按照阻塞状态的情况来进行处理
         如果线程的当前状态处于阻塞状态，那么在将中断标志设置为true后，如果是wait、sleep以及jion三个方法引起的阻塞，那么会将线程的中断标志重新设置为false，并抛出一个InterruptedException；

         例如，一个线程在运行状态中，其中断标志被设置为true,则此后，一旦线程调用了wait、jion、sleep方法中的一种，立马抛出一个InterruptedException，且中断标志被清除，重新设置为false。
         总结，调用线程类的interrupted方法，其本质只是设置该线程的中断标志，将中断标志设置为true，并根据线程状态决定是否抛出异常。
         因此，通过interrupted方法真正实现线程的中断原理是：开发人员根据中断标志的具体值，来决定如何退出线程。
   ...
   * 阻塞：等待，sleep，io阻塞，同步阻塞
      三种原因引起阻塞：等待阻塞、同步阻塞以及其他阻塞（睡眠、jion或者IO阻塞）；
      等待阻塞是调用wait方法产生的，
...
11. java的内存模型
      ...
      JMM(内存模型)定义了线程和主内存之间的抽象关系：
            ...
            内存模型对应堆栈中的概念
               ...
               2.把堆内存变量的具体值 load 到线程本地内存中，建立一个变量副本，
               3.线程就不再和对象在堆内存变量值有任何关系，而是直接修改副本变量的值，
               4.在修改完之后的某一个时刻（线程退出之前），自动把线程变量副本的值回写到对象在堆中变量。堆中的对象的值就产生变化
...
12. 同步辅助类
	...
	* Semaphore（信号量）
         ...
         semaphore方法
            (常用)availablePermits（）返回此Semaphore对象中当前可用的许可数，许可的数量有可能实时在改变，并不是固定的数量。
            drainPermits（）可获取并返回立即可用的所有许可个数，并且将可用许可置0。

   * 控制线程的顺序

      * 方法1 semaphore
            核心思路是，控制两者的顺序就要引入一个Semaphore(0)，用release 和 acquire可控制顺序，先执行的release要放逻辑最后
            该例子暂保证a和b,c的顺序，简化demo。了解到两层顺序就可以，三层本质思想没变，但是有点绕了，不典型

            确保b,c在a之后完成，b,c中获取，acquire()，若没有可用则等待

            public static void main(String[] args) throws InterruptedException {
               final Semaphore semaphore = new Semaphore(0);  
               // final Semaphore semaphore1 = new Semaphore(0);       //保证a,b,c执行顺序用

               Thread threada = new Thread(
                     new Runnable() {
                        public void run() {
                           System.out.println("执行线程1开始----"+semaphore.availablePermits());   //日志1 这是合理的位置
                           semaphore.release(); //步骤1
                        // System.out.println("执行线程1结束----"+semaphore.availablePermits());   //日志1，这里位置不对
                        }
                     });
               Thread threadb = new Thread(
                     new Runnable() {
                        public void run() {
                           try {
                              semaphore.acquire();                //步骤2
                           } catch (InterruptedException e) {}
                           System.out.println("执行线程2----"+semaphore.availablePermits());    //日志2
                           semaphore.release();		//这里是为了让另一个等待的执行，这样可以让b和c在a之后执行，但不能保证b和c的顺序
                           //semaphore1.release();   //保证a,b,c执行顺序用
                        }
                     });
               Thread threadc = new Thread(
                     new Runnable() {
                        public void run() {
                           try {
                              semaphore.acquire();
                              semaphore.release();    //保证a,b,c执行顺序用，这里释放锁，防止相互等待造成死锁
                              semaphore1.acquire();   //保证a,b,c执行顺序用
                           } catch (InterruptedException e) {}
            ...
            这里如果先步骤2后步骤1，但是日志打印线程无序，会出现步骤2等1，等到后立马输出日志2，然后日志1，所以最好日志1要在步骤1之前，即业务在步骤1之前先执行

      * 方法2 CountDownLatch
            核心思路是，new CountDownLatch(1)，然后用countDown() 和  await() 实现顺序
...
14. 多线程的实现
      ...
      * 继承Thread类
      ...
      简化写法(匿名子类)
       创建thread也可以使用子类override覆盖thread类中run方法的方式
         new Thread() {
                  @Override
                  public void run() {
                     System.out.println("这怎么说呢");
                  }
               }.run();
...
20. Thread重复调用start方法
      ThreadTest thread1 = new ThreadTest();
      thread1.start();
      thread1.start();
      thread1.start();

      报错IllegalThreadStateException异常
      ...
      原因：
         ...
         原因是：同一个Thread重复调用了start方法，如果你想开启多个线程，并且共享资源的时候，有两个方法
            1.把共享资源设置为static
            2.使用runnable
               将自己定义的SellTicket extends Thread 线程类改成SellTicket implements Runnable，后用new Thread(robot).start 就可以启动多次了

            for中调用同一个thread对象不可以，应该是for中每次都new一个thread，其中runnable是固定的一个
...
#### 锁基本概念
   ...
   几种锁类型
      ...
      偏向锁：在锁对象的对象头中记录⼀下当前获取到该锁的线程ID，该线程下次如果又来获取该锁就可以直接获取到

      自旋锁是线程通过CAS获取预期的一个标记，如果没有获取到，则继续循环获取，如果获取到了则表示获取到了锁，这个过程线程一直在运行中，没有使用太多的操作系统资源，比较轻量。
      自旋锁要占用CPU，如果是计算密集型任务，尽量减少锁的使用。若锁竞争时间较长，自旋通常不能获得锁，浪费自旋占用的CPU时间。通常发生在锁持有时间长，竞争激烈的场景中，此时应主动禁用自旋锁

      聊聊偏向锁，轻量级锁的原理和过程

      偏向锁、轻量级锁、重量级锁，是对synchronized的锁优化(jdk1.6后提供)

      *****
         synchronized最开始的时候并不是上来就是一把重量级锁，而是随多线程并发竞争锁的激烈程度提高来不断升级，慢慢变成重量级锁，整个升级过程会经历偏向锁、轻量级锁、自旋锁、重量级锁的过程。

         优化顺序，锁会随着线程的竞争情况逐渐升级，偏向锁 => 轻量级锁 => 重量级锁 。锁可升级不能降级。
         轻量级锁是由偏向锁升级来的，偏向锁运行在一个线程进入同步块的情况下，
         当第二个线程加入锁竞争的时候，偏向锁就会升级为轻量级锁(需撤销偏向锁)；
         若轻量级锁自旋到达阈值后，没有获取到锁，就会升级为重量级锁；
      *****

      对象头：synchronized用的锁是存在Java对象头里的。Java对象头里的Mark Word记录了对象和锁有关的信息(默认存储对象的HashCode、分代年龄和锁标记位),主要用来表示对象的线程锁状态。

      偏向锁
         无自旋锁优化
         偏向锁只适合大部分锁没有被竞争的系统中（也可以说是只有一个线程访问同步块的场景），如果系统中存在大量被争用的锁时，会导致持有锁的线程不断切换，这时可以考虑关闭偏向锁。
         
         偏向锁，偏向于第一个访问锁的线程，若在运行过程中，无多线程竞争(同步锁只有一个线程访问)，则线程是不需要触发同步的，此时会给线程加一个偏向锁。
         如果在运行过程中，遇到了其他线程抢占锁，则持有偏向锁的线程会被挂起，JVM会消除它身上的偏向锁，将锁恢复到标准的轻量级锁。

         适用场景：无实际竞争，且将来只有第一个申请锁的线程会使用锁。
         
      轻量级锁
         场景是在不同的时间段有不同的线程请求锁（线程交替执行）
         适用场景：少量线程交替获取锁，同步块执行速度非常快的场景。追求响应时间。
         
         轻量级锁每次申请、释放锁都至少需要一次CAS，但偏向锁只有初始化时需要一次CAS。
         
         等待轻量锁的线程不会阻塞，它会一直自旋等待锁，并如上所说修改markword。
         自旋锁，未获得锁时，不被挂起，而转而去执行一个空循环，即自旋。若干自旋后，如果还没有获得锁，则才被挂起，使用重量级锁，则执行代码。

         适用场景：无实际竞争，多个线程交替使用锁；允许短时间的锁竞争(适量自旋)。
         
      重量级锁
         是我们平常说的加的同步锁，也就是java基础的锁实现，获取锁与释放锁的时候都要进行系统调用，从而导致上下文切换。
         重量级锁，所有来加锁的线程加锁失败之后，就会加入等待的队列中，等待别的线程释放锁之后再进行锁的竞争。

         适用场景：大量线程同时竞争锁，追求吞吐量。
                  有实际竞争，且锁竞争时间长。
                  如果锁竞争时间短，可以使用自旋锁优化轻量级锁性能，减少线程切换。
                  如果锁竞争程度逐渐提高，从偏向锁逐步膨胀到重量锁，能够提高系统的整体性能。
      优缺点对比
         偏向锁   
            优：加锁/解锁不需要额外的消耗
      ...
      线程锁的相关概念：
		...
		线程中的集中锁的分类：
		1、重入锁
            当一个线程执行到某个 synchronized 方法时，比如说 method1，而在 method1 中会调用另外一个 synchronized 方法 method2，此时线程不必重新去申请锁，而是可以直接执行方法 method2。
		2、可中断锁
            如果某一线程A正在执行锁中代码，另一线程B正在等待获取该锁，可能由于等待时间过长，线程B不想等待了，想先处理其他事情，我们可以让它中断自己或者在别的线程中中断它，这就是可中断锁
   ...
   * 2. 自旋锁(spin lock)
         ...
         自旋锁
            使用场景：锁竞争不激烈,锁持有时间较少情况下的，自检等待
            可以避免线程切换,自旋的消耗cpu相对较小，反之不适合，因为自旋锁在获取锁前一直都是占用 cpu 做无用功等待,需设定一个自旋等待的最大时间 
   ...
   * 3. 公平锁
         ...
         ReentrantLock 和 ReentrantReadWriteLock(实现的是ReadWriteLock接口)，默认的lock()方法采用的是非公平锁，但是可以设置为公平锁
...
#### lock锁和synchronized同步锁，Lock 和 synchronized
   ...
   3. synchronized
         ...
         * 2)线程执行发生异常，此时 JVM 会让线程自动释放锁。
         ...
         synchronized优化(忽略)
            ...
            Java1.7与1.8 中，均对该关键字的实现机理做了优化。引入了偏向锁和轻量级锁。
   ...
   4. Lock 和 synchronized 的区别
      ...
      7. synchronized的锁可重入、不可中断、非公平，而Lock锁可重入、可中断、可公平（两者皆可，可选）
      ...
      lock 和 synchronized，两种锁锁方法
		...
		AbstractQueuedSynchronized（AQS）(忽略)
         重点是状态变量state，同步队列Sync queue(FIFO，Node节点组成)
         ...
         AQS 定义两种资源共享方式
            Exclusive 独占资源,独占锁-ReentrantLock
               Exclusive（独占，只有一个线程能执行，如 ReentrantLock）
            Share 共享资源，共享锁-Semaphore/CountDownLatch
               Share（共享，多个线程可同时执行，如 Semaphore/CountDownLatch）。

               Semaphore/CountDownLatch/ReentrantLock 都有一个静态内部类，static class Sync extends AbstractQueuedSynchronizer。

         AQS内部维护了一个CLH队列(FIFO)来管理锁，用volatile修饰共享变量state，线程通过CAS去改变状态符，成功则获取锁成功，失败则进入等待队列，等待被唤醒。

         AQS是将每一条请求共享资源的线程封装成一个CLH锁队列的一个结点（Node），来实现锁的分配。	
            线程先尝试获取锁，若失败就将当前线程及等待状态等信息包装成一个node节点加入到同步队列sync queue里。 
            接着不断循环尝试获取锁，条件是当前节点为head的直接后继才会尝试。
            如果失败就会阻塞自己直到自己被唤醒。而当持有锁的线程释放锁时，会唤醒队列中的后继线程。

         (1)状态变量state；
            表示对象的当前状态（如0表示unlock，1表示lock）
            state的三个方法，getState();setState();compareAndSetState();
            
            在AQS中维护了一个 volatile 修饰的状态信息 state，
            对于ReentranLock，state表示当前线程获取锁的可重入次数；
            对于CountDownLatch，state表示当前计数器的值；
         ...
         state初始化为0(未锁定状态)，A线程lock()时，会调用tryAcquire()独占锁并将state+1,其他线程再tryAcquire时会失败，直到A线程unlock()到state=0为止，其他线程才有机会获取该锁。
         A释放锁之前，自己也是可以重复获取此锁（state累加），这属于可重入。
         注意：获取多少次锁就要释放多少次锁，保证state是能回到零态的。
         lock和unlock操作了，对应到AQS中为acquire、release方法，
   ...
   6. ReentrantLock提供的获取锁的方式
            ...
            注意这里的lock的对象一定要是同一个对象(同一把锁)，这样才会资源竞争，发生冲突。

			2.tryLock() 	
				用来尝试获取锁，如果获取成功，则返回true
            只在锁空闲的时候才获取这个锁，否则返回 false，所以它不会 block代码的执行
               案例代码
                  Lock lock = ...;
                  lock.lock();
                  try{
            ...
            如果当前线程没有被中断，就获取锁；否则抛出InterruptedException，并且清除中断
            ...
            线程的中断lockInterruptibly 和interrupt
            lockInterruptibly()，当通过这个方法去获取锁时，如果线程正在等待获取锁(暂时获取不到，处于阻塞等待状态)，则这个线程能够响应中断，即中断线程的等待状态。
            当两个线程同时通过 lock.lockInterruptibly()想获取某个锁时，假若此时线程A获取到了锁，而线程B只有在等待，那么对线程B调用threadB.interrupt()方法能够中断线程B的等待过程。

            由于 lockInterruptibly()的声明中抛出了异常，所以 lock.lockInterruptibly()必放在 try 块中或者在调用 lockInterruptibly()的方法外声明抛出InterruptedException。
            ...
            1.线程处于阻塞状态：如使用了 sleep,同步锁的 wait,(或socket中的receiver/accept等方法时)，会使线程处于阻塞状态。
               当调用线程的 interrupt()方法时，会抛出 InterruptException 异常。通过代码捕获该异常，然后break跳出循环状态,让我们有机会结束这个线程的执行(正常结束 run 方法)。
   ...
   7. lock中的condition(忽略)
         ...
         Condition也可以看做是对象 wait/notify/notifyAll方法的替代。
         Condition是在java 1.5中才出现的，用来替代传统的Object的wait()、notify()实现线程间的协作，相比前者，使用Condition1的await()、signal()这种方式实现线程间协作更加安全和高效。
         简单说，作用是使得某些线程一起等待某个条件（Condition），只有当该条件具备(signal 或者 signalAll方法被调用)时，这些等待线程才会被唤醒，从而重新争夺锁。
         ...
         注意事项
            ...
            Conditon中的await()、signal()、signalAll()对应Object中的wait()、notify()、notifyAll()；

         使用场景，可以控制唤醒启动的线程顺序
            线程A(wait)-->线程C(wait)-->线程B(notify) //没毛病，因为wait后是释放了锁的，这里可能A和C都在等待中
            
            问题：同一个obj对象，等待的线程中有A和C, B在notify后只会唤醒其中一个执行(notifyAll同样只有一个执行)；若想让A线程执行，object的方式是无法控制的，需要引入condition
            ...
            多线程环境的下，线程直接的互斥[执行]依靠的应该是锁Lock，线程的之间的[通信]依靠的应该是条件Condition/信号，
            一般情况下lock确实可以同时满足做这两个事情，需要让满足一定条件的线程执行，仅仅依靠锁是不能完美解决的。
            所以condition实际上分离了执行和通信。 Object的wait等方法做不到
            
            condition是对wait方法的补充，可以选择唤醒顺序(同一把lock下的多个condition，可自定义唤醒顺序)
         ...
         一个lock对个condition的唤醒问题
            在一个同步程序中，如果定义了一个Lock锁，同时在这一个锁上创建两个condition监视器con1和con2，
            如果操作con1.signalAll()，那只是唤醒是在Lock这个锁里，被con1这个监视器await的线程(con1这个对象上的阻塞队列里的线程)
   ...
   9. 锁统一对象，资源竞争
         ...
         1.这里可以将obj通过构造函数方式传入runnable对象中
         2.可将obj设计成final类型，这样可以使用匿名内部类简化线程启动写法，将锁的方法抽象到obj对象中去实现
      ...
      * Executors是线程池的工厂类
            ...
            Executors 类提供工厂方法创建不同类型的线程池(本质上使用了 ThreadPoolExecutor 的构造方法创建线程池)。
               1.newSingleThreadExecutor()?创建一个只有一个线程的线程池，
                     corePoolSize 和 maximumPoolSize都设置为1，使用LinkedBlockingQueue

               2.newFixedThreadPool(int numOfThreads)来创建固定线程数的线程池，
                     corePoolSize 和 maximumPoolSize值是相等的，使用LinkedBlockingQueue

               3.newCachedThreadPool()   缓存线程池，线程数量不限制，可能oom
                     corePoolSize设置为0，将maximumPoolSize设置为Integer.MAX_VALUE，使用SynchronousQueue
...
5. future相关 和 futuretask相关，
      ...
      使用场景
		...
		future.get()方法的阻塞优化，CompletionService方案
         ...
         CompletionService是对ExecutorService的改进，类似Queue功能，可调用poll()获取结果对象，还有一个take方法(没有结果对象时返回空,而poll方法会block住线程直到有结果对象返回)
...
6. 线程池的工作原理(忽略)：
      ...
      可以继承重写Thread 类，在其 start 方法中添加不断循环调用传递过来的 Runnable 对象。 
...
### 线程变量Threadlocal，ThreadLocal相关
   ...
   本质线程共享使用 private static final ThreadLocal<T>变量 实现线程私有属性的操作，每次操作线程变量即可。
   ...
   threadLocal一般都是声明在静态变量中，如果不断的创造threadLocal而没有调用remove方法，会导致内存泄漏(其实线程结束后可自动释放相关threadlocal变量)
...
6. 线程中使用传值
   ThreadLocal这种变量在线程的生命周期内起作用，减少同一个线程内多个函数或者组件之间一些公共变量的传递的复杂度
   将一个共用的 ThreadLocal静态实例作为 key，将不同对象的引用保存到不同线程的ThreadLocalMap 中，
   然后在线程执行的各处通过这个静态ThreadLocal实例的 get()方法取得自己线程保存的那个对象，避免了将这个对象作为参数传递的麻烦。
...
* 并发的三个特性
      1、原子性：
            ...
            增量操作，i++操作不是原子操作，经历了3个操作(读-改-写)， 1.获取变量当前值,  2.为该值加1, 3.写回更新值
...
* 普通锁和分布式锁区别
      ...
      ReentrantLock 的 lock 和 unlock 要求必须是在同一线程进行，是成对出现的。
      ...
      lock和synchronized都是针对同一个jvm中的资源锁(对象，类),跨系统自然不适用了。就是说这个资源不在固定的一个jvm中，而是分布式环境中，是多个jvm共享的。
...
* 分布式锁常用方案：
   ...
   1. 基于Redis的分布式锁(推荐)
         ...
         * 使用redission实现
            ...
            分布式锁，https://blog.csdn.net/asd051377305/article/details/108384490

            Redisson、Jedis、Lettuce是三个不同的操作Redis的客户端，
            Jedis、Lettuce的API更侧重对Reids数据库的CRUD(增删改查)，而Redisson API侧重于分布式开发

            redisson的源码实现可以发现，不管是加锁、解锁、续约都是客户端把一些复杂的业务逻辑，通过封装在Lua脚本中发送给redis，保证这段复杂业务逻辑执行的原子性。


            锁续期，自动续期，看门狗机制，
               若用setnx做分布式锁，难以实现锁续期，比如锁过期时间10s，而业务处理花了30s，而redisson可以处理恰好处理这类锁续期的问题
               
               锁过期的问题，我们可以在加锁的时候，手动调长redis锁的过期时间，可这个时间多长合适？业务逻辑的执行时间是不可控的，调的过长又会影响操作性能。


               Redisson通过watch dog(看门狗机制)解决了锁自动续期的问题


               默认锁过期时间是30s，自动续期源码中开启了定时任务watchdog，定期检查去续期,定时调度每次调用的时间差是lockWatchdogTimeOut/3，
               在业务没有处理完时，锁默认每隔10s续期到30s；业务执行完，那么你应该手动释放锁，即使没有手动释放锁，到期后，锁自动失效。
               
               watchdog看门狗机制    https://wenku.baidu.com/view/a6b4ceb368ec0975f46527d3240c844768eaa04c.html 

               延期的作用是，保证在业务执行完之前，不会锁过期,业务完成之后锁会自动过期,有个过期时间解决了死锁
               watchdog机制启动，且代码中没有释放锁操作时，watch dog 会不断的给锁续期。

               不指定过期时间，同步锁默认时间为lockWatchdogTimeOut【30s】,每30/3=10s就自动续一次锁


               看门狗自动续期原理
               Redisson看门狗机制，只要客户端加锁成功，就会启动一个 Watch Dog。leaseTime 必须是 -1 才会开启 Watch Dog 机制
               如果需要开启 Watch Dog 机制就必须使用默认的加锁时间。如果你自己自定义时间，超过这个时间，锁就会自定释放，并不会自动续期。
               指定时间会直接去加锁，没有指定会先加锁(默认是lockWatchdogTimeOut  30s)，

               watchdog只有在未显示指定加锁时间(leaseTime)时才会生效
               lockWatchdogTimeOut设定不要太小，不要设置太小，网络延迟一下，key直接过期了
            ...
            * 实例代码
               ...
               锁实例
                  public void testReentrantLock(RedissonClient redisson){
                     RLock lock = redisson.getLock("anyLock");
                     try{
                        //lock.lock();												// 1. 最常见的使用方法
                        //lock.lock(10, TimeUnit.SECONDS);							// 2. 支持过期解锁功能,10秒钟以后自动解锁, 无需调用unlock方法手动解锁
                        boolean res = lock.tryLock(3, 10, TimeUnit.SECONDS);		// 3. 尝试加锁，最多等待3秒，上锁以后10秒自动解锁
                        if(res){ //成功
                           //do your business
                        }
                     } catch (Exception e) {
            ...
            * 配置项
               ...
               public class RedissonTest {
                  ...
                  static {
                     Config config=new Config();
                     config.useSingleServer().setAddress("redis://192.168.6.251:6379");
            ...
            * 常用方法
                  ...
                  config.setLockWatchdogTimeout(7000);  //配置自定义的时间

                  直接lock会阻塞，单独tryLock()不会阻塞，会立刻返回获取结果
                 
                  lock.lock();							//阻塞获取锁等待默认30s，有watchdog功能自动续期
                  lock.lock(leaseTime,TimeUnit.SECOND)	//阻塞方法，不会自动续期，过期时间为leaseTime，超过时间自动解开，无法自动续期
                  rLock.tryLock(10,TimeUnit.SECONDS);		//尝试拿锁10s后停止重试返回false，会自动续期

 

            * 常见异常
                  Exception in thread "main" java.lang.IllegalMonitorStateException: attempt to unlock lock, not locked by current thread by node id

                  异常主要原因
                     1.解锁需要创建锁的线程进行解锁
                     2.过期时间默认30S，超过30S秒，即使用同一个线程解锁也会报这个异常，因为锁已经不存在了
                     3.只有加锁成功才需要解锁、

               
            Redis分布式锁的缺点（忽略）
               如果对某个redis master实例，写入myLock这种锁key的value，此时会异步复制给对应的master slave实例。
               但是这个过程中一旦发生redis master宕机，主备切换，redis slave变为了redis master。
               接着就会导致，客户端2来尝试加锁的时候，在新的redis master上完成了加锁，而客户端1也以为自己成功加了锁。
               此时就会导致多个客户端对一个分布式锁完成了加锁。导致脏数据的产生。
               所以这个就是redis cluster，或者是redis master-slave架构的主从异步复制导致的redis分布式锁的最大缺陷：在redis master实例宕机的时候，可能导致多个客户端同时完成加锁
               

            红锁(忽略)
               实际场景中红锁很少使用。使用后会影响高并发环境下的性能，一般要保证Redis集群的可靠性。
               同时，使用红锁后，当加锁成功的RLock个数不超过总数的一半时，会返回加锁失败，即使在业务层面任务加锁成功了，但是红锁也会返回加锁失败的结果。
               另外，使用红锁时，需要提供多套Redis的主从部署架构，同时，多套Redis主从架构中的Master节点必须都是独立的，相互之间没有任何数据交互。
   ...
   2. 基于数据库资源表的分布式锁(不常用)
            ...
            分布式锁的使用数据库乐观锁，本质也是提交时判断冲突，版本号，不同线程副本更新的版本号不一致。
   ...
   3. 基于ZooKeeper的分布式锁(不常用)
            ...
            利⽤的是zookeeper的临时节点、顺序节点、watch机制来实现的，zookeeper分布式锁的特点是高一致性，
...
* 其他   
   ...
   * 同时还要考虑到redis节点挂掉后的情况，所以需要采用红锁的方式来同时向N/2+1个节点申请锁，都申请到了才证明获取锁成功，这样就算其中某个redis节点挂掉了，锁也不能被其他客户端获取到
      ...
      红锁的原理
      红锁本质上就是使用多个Redis做锁。例如有5个Redis，一次锁获取，会对每个请求都获取一遍，如果获取锁成功的数量超过一半(2.5)，则获取锁成功，反之失败；
      释放锁也需要对每个Redis释放
...
* 2、数据库解决方案：
    	...
    	数据库的乐观锁
      去重表(不实用)，将业务中有唯一标识的字段保存到去重表，如果表中存在，则表示已经处理过了
...
3. 数据库中的锁
		...
		从并发事务锁定的关系上看：
            ...
            加SELECT FOR UPDATE 语句，表明凡是读到的数据，我会加排他锁，防止其他数据同一时间获取相同数据) 
         ...
         共享锁(读锁)（忽略）
            ...
            对于共享锁： 如果两个事务对同一个资源上了共享锁，事务A想更新该数据，那么它必须等待事务B释放其共享锁。
...
6. 索引相关
   ...
   8.  * 树形结构
         叶子结点  就是没有子结点的结点，又称为终端结点。这里叶子节点，是指为空(NIL或NULL)的叶子节点。
         分层结构是 		根节点-非叶子节点-叶子节点

         * 二叉树
            二叉树特点是每个节点最多只能有两棵子树(子节点)，且有左右之分，左小右大 
         
            搜索方法
               从根结点开始，如果查询的关键字与结点的关键字相等，那么就命中；
               如果查询关键字比结点关键字小，往左；反之，往右；
               如果左/右子节点的指针为空，则找不到相应的关键字；
            ...
            平衡二叉树(忽略)
               为保持B树结点分布均匀,在原二叉搜索树的基础上加上平衡算法(一种在二叉搜索树中插入和删除结点的策略)
                     

         * b-树(b树)
            类似普通的平衡二叉树，区别是B-树允许每个节点有更多的子节点，这里节点里存储着索引值和具体data
            关键字集合分布在整颗树中（重点）,且任何一个关键字只能出现在一个结点中
            一个结点有k个孩子时，必有k-1个关键字才能将子树中所有关键字划分为k个子集。这里限制了节点的元素数量

            b树中，B 树中每个节点包含了键值和键值对应的数据对象存放地址指针，所以成功搜索一个对象可以不用到达树的叶节点。
            
            搜索
               B-树的搜索，从根结点开始，对结点内的关键字（有序）序列进行二分查找，
               如果命中则结束，否则进入查询关键字所属范围的子结点；重复，直到叶子结点；
               搜索有可能在非叶子结点结束
         
         
         * b+树
            非叶子结点相当于是叶子结点的索引，叶子结点相当于是存储（关键字）数据的数据层(而b树是每个关键字都保存数据)
            
            所有的非叶子结点可以看成是索引部分，结点中仅含其子树中的最大（或最小）关键字。
            非叶子结点的子树指针与关键字个数相同
            
            相对b树的优点(重点)
               1.叶子结点使用链表相连，便于区间查找和遍历。B树则需要进行每一层的递归遍历
               2.b+树的中间节点不保存数据，能容纳更多节点元素，b+树的索引层数比B树少，b+树更加“矮胖”，io操作更少，因此检索速度比B树快

            和b树的其他区别
               有k个子树的中间节点包含有k个元素（B树中是k-1个元素）
               所有的中间节点元素都同时存在于子节点(重复性)，在子节点元素中是最大/小元素（b树中节点的元素不重复出现）

            B+树的多分支结构有效降低了树的高度，相比二叉树来说高度降低了很多，能够有效的减少磁盘 IO，
         ...
         * 几种结构的区别
            ...
            B（B-）树：多路搜索树，每个结点存储M/2到M个关键字，非叶子结点存储指向关键字范围的子结点，只要找到匹配元素即可（无论处于中间节点还是叶子节点），
            B+树：在B-树基础上，为叶子结点增加链表指针，所有关键字都在叶子结点中出现，非叶子结点作为叶子结点的索引；B+树总是到叶子结点才命中
            B*树：在B+树基础上，为非叶子结点也增加链表指针，在B+树的非根和非叶子结点再增加指向兄弟的指针，提高效率
   ...
   9. b+树中叶子节点存储内容
      ...
      数据存在磁盘的一个文件里面，ibd文件，ibd里面既有索引数据（非叶子节点Page），也有具体的数据（叶子结点Page）。
      主键索引和具体数据一起组成了B+树，所以说数据存在B+树里面。
      InnoDB数据是存在B+树这一点没错，只不过是存在主键索引构成的B+树，普通索引的叶子节点存的是主键id

      联合索引的b+树
      INDEX(COL3, COL2)
      如果对 col3 和 col2 建立 联合索引，肯定只有一棵B+ 树，又因为 最左原则的存在，会优先根据col3 排序，在根据 col2 排序。
      所以建立联合索引，能更大可能实现索引覆盖，因为包括的索引字段变多了
      https://blog.csdn.net/klchht/article/details/78146443
...
7. mysql索引结构，sql优化，limit优化，回表，索引覆盖
      ...
      如果没有索引，每个数据项都要发生一次IO，那么总共需要百万次的IO，有了索引，只要几层的树形搜索就能找到了。相当于目录页
	...
	* 聚集索引 和 普通索引 的区别
         ...
         类似字典中的拼音就是聚集索引，偏旁部首查汉字，就是非聚集索引。
            ...
            聚集索引的叶子节点就是对应的数据节点，叶子节点中保存存了索引列和具体的数据(非聚集索引的叶节点仍然是索引节点)
	...
	* 覆盖索引 和 回表
         ...
         从非主聚簇索引中就能查到的想要数据，而不需要通过回表从主键索引中查询其他列
			尽量不要使用select *， 会导致回表问题，使用select id 则不会
      ...
      **********
         索引查询流程
            主键查询流程，是直接查询到主键索引树的叶子节点，能拿到全部数据
            普通索引，要先在自身索引树中的叶子节点拿到主键id，想获取整行数据还要去主键索引树中取查询，
	...
	* 大数据量的limit优化思路，千万分页，百万分页
         ...
         *****对limit的优化，不是直接使用limit，而是首先获取到offset的id，然后直接使用limit size来获取数据。*****
         ...
         explain了一下，才发现mysql LIMIT 10000, 20的意思扫描满足条件的10020行，扔掉前面的10000行，返回最后的20行
         越往后面翻页越相当于全表扫描，这样在一个高并发的应用下，性能肯定是hold不住的。但是LIMIT n这样的语句性能是没有问题的，因为只扫描n行
         这相当于越往后面翻页越相当于全表扫描，这样在一个高并发的应用下，性能肯定是hold不住的。但是LIMIT n这样的语句性能是没有问题的，因为只扫描n行。

         EXPLAIN
         SELECT * FROM ac_pay_acct_txn  ORDER BY id ASC LIMIT 500,10
         //这种只扫描了510行，不是之前想得那样先查出结果在截取(逻辑分页)，实测是物理分页，查询的扫描的范围就510行，然后截取10条出来
         ...
         Solution3:使用JOIN分页方式，这个没有尝试过，效果同2
...
8. 数据库建表的经验：
   ...
   8. Where 子句中经常使用的字段应该创建索引，分组字段或者排序字段应该创建索引，两个表的连接字段应该创建索引

   9. 索引字段尽量使用not null字段

   10. 从 innodb 的索引结构分析，为什么索引的 key 长度不能太长？
         key 太长会导致一个页当中能够存放的 key 的数目变少，间接导致 索引树的页数目变多，索引层次增加，从而影响整体查询变更的效率
...
10. 常见数据库架构
    ...
    * 常见方案
    ...
    ​	   4. 方案四：双主+主从架构，看似完美的方案
         ...
         网站利用数据库的这一功能，实现数据库读写分离，从而改善数据库负载压力
...
11. mysql分表相关，分库相关，分库分表相关
      ...
      分布式数据库是网站数据库拆分的最后手段，只有在单表数据规模非常庞大的时候才使用（水平拆分）。
      不到不得已时，网站更常用的数据库拆分手段是业务分库，将不同业务的数据部署在不同的物理服务器上（垂直拆分推荐）。
...
20. 单纯的select * from a,b 是笛卡尔乘积
      ...
      关联查询
         ...
         join时，应用小的结果驱动大的结果，left join左表结果尽量小
...
21. mysql优化
      ...
      where 语句中索引独立出现，索引才会起作用，不要放在表达式中(如: 转换函数一般放在值那边，不要放在列那边)，或发生不合适的隐式转换
		...
		几个禁止
			...
			禁止使用OR条件，必须改为IN查询解读：旧版本Mysql的OR查询是不能命中索引的
			where 字句有 or 出现还是会遍历全表。
         避免where使用or来连接条件，这样不走索引，等价的优化方法是使用union all来代替
      ...
      用UNION 替换 OR (适用于索引列)
      通常情况下, 用 UNION 替换 WHERE 子句中的 OR 将会起到较好的效果. 对索引列使用 OR将造成全表扫描. 
...
6. 网址解析步骤
   ...
   浏览器查询域名的IP
      本地DNS服务器，如果本地DNS服务器上没有的话，它会递归的从根DNS服务器、顶级DNS服务器、权威DNS服务器请求，然后把获取到的IP返回给浏览器（DNS 协议基于 UDP）。


   DNS负载均衡采用的是简单的轮询算法，不能区分服务器之间的差异，不能反映服务器当前运行状态，所以其的负载均衡效果并不是太好。

   这里的域名解析
      先根据URL域名从本地hosts文件查找是否有映射IP，如果没有就将域名发送给电脑所配置的DNS进行域名解析，得到IP地址，途中可能会经过各种路由器、交换机，最终到达服务器
...
13. 普通的负载均衡软件，如 LVS，其实现的功能只是对请求数据包的转发、传递，从负载均衡下的节点服务器来看，接收到的请求还是来自访问负载均衡器的客户端的真实用户；
    而反向代理就不一样，反向代理服务器在接收访问用户请求后，会代理用户重新发起请求代理下的节点服务器，最后把数据返回给客户端用户。
    在节点服务器看来，访问的节点服务器的客户端用户就是反向代理服务器，而非真实的网站访问用户。
...
## java反射相关	
   ...
   在Java中的反射机制是指在运行状态中，对于任意一个类都能够知道这个类所有的属性和方法； 并且对于任意一个对象，都能够调用它的任意一个方法；
   这种动态获取类中信息以及动态调用对象方法的功能成为Java语言的反射机制。
...
6. ​反射生成Java对象实例
      * 通过类对象调用newInstance()方法，适用于无参构造方法：（常用），要求该Class对象对应的类有默认的空构造器。
...
7. java反射的主要属性
        ...
        Class 类 
            ...
            对象的 newInstance()方法来创建该 Class 对象对应类的实例，但是这种方法要求该 Class 对象对应的类有默认的空构造器
...
8. 反射本身的field可以直接设置，也可以通过setMethod.invoke()设值
      ...
      也可选定构造器创建对象
         ...
         Person p=(Person) clazz.newInstance();  //使用.newInstane 方法创建对象
         
         Constructor c=clazz.getDeclaredConstructor(String.class,String.class,int.class);//获取构造方法并创建对象
...
* 7. logback中   appender是打印器  需要绑定到logger中  而root是根logger
      案例：
         ...
         避免重复打印日志，浪费磁盘空间，务必设置 additivity=false。
...
### Elasticsearch
      ...
      相关介绍    https://blog.csdn.net/qq_21244099/article/details/88959217
   ...
   * 分词
      ...
      elasticsearch 里默认的IK分词器是会将每一个中文都进行了分词的切割，所以你直接想查一整个词，或者一整句话是无返回结果的。
		...
		关于分词
         ...
         standard分词，基本就是将中文分成一个个单字
         ik分词，可以将中文，多分成几个词语的组合，更适用中文，ik_smart:尽可能少的进行中文分词。ik_max_word:尽可能多的进行中文分词。
   ...
   * es 初始化配置相关代码段
         这里是初始化服务索引库的代码，可以手工执行一次，即可代码创建对应es中的索引配置信息，其他的索引配置类似
               ...
               XContentBuilder mappings = JsonXContent.contentBuilder()
                     .startObject()
                        .startObject("properties")
                           .startObject("req_title")                    //这里可以创建多个索引字段
                              .field("type","text")
      ...
      ```
         //2.0先创建索引，后更新索引
            ...
            try {
               ...
               String esHost = "192.168.6.50";
               RestHighLevelClient client = null;
               client = new RestHighLevelClient(RestClient.builder(new HttpHost(esHost, 9200, "http")));
               GetIndexRequest request = new GetIndexRequest(Es.INDEX_PAY_FEE);
               // 查询存在
               boolean user = client.indices().exists(request, RequestOptions.DEFAULT);
               System.out.println(Es.INDEX_PAY_FEE + "是否存在" + user);
               if (!user) {
                  // 创建
                  CreateIndexRequest requestCreate = new CreateIndexRequest(Es.INDEX_PAY_FEE);
                  client.indices().create(requestCreate, RequestOptions.DEFAULT);
               }
               putMappings(client);
            } catch (Exception e) {
               e.printStackTrace();
         ...
         * @throws IOException
         */
         public static void putMappings(RestHighLevelClient client) throws IOException {

            try {
               PutMappingRequest request = new PutMappingRequest(Es.INDEX_PAY_FEE);
               XContentBuilder builder = XContentFactory.jsonBuilder()
                     .startObject();
                        .startObject("properties");
                           .startObject("orderSn").field("type", "text").endObject()
                           .startObject("containsPayFee").field("type", "boolean").endObject()
                        .endObject();
                     .endObject();
   ...
   * 初始化client配置
      public class Es {
         ...
         public static boolean conflictFlag=true;  //启动加载标记
         
         static {
            HttpHost httpHost = new HttpHost("192.168.6.251", 9200, "http");
            RestClientBuilder restClientBuilder = RestClient.builder(httpHost);
            client = new RestHighLevelClient(restClientBuilder);
            
            if(StringUtil.isBlank_new(Constants.URL_ES)) {
               conflictFlag=false;//首次加载标记
            }
         }
         
         /**
         * 启动，mq消费先，读取地址为空，导致es的client初始化地址为空。
         * @return
         */
         ...
         public static final String INDEX_SERVICE = "service";
      }
...
#### Elasticsearch: 权威指南	   处理人类语言，词汇识别，标准分析器          
   ...
   * 常用api操作
         ...
         后端全写
            curl -XGET 'localhost:9200/_count?pretty' -d '				#计算集群中文档的数量，es查询中，任意查询字符串中添加pretty参数，es可以得到易于识别的json结果
            {
               "query": {
                  "match_all": {}
               }
      ...
      *****我们的应用程序根本不应该关心分片，对于应用程序而言，只需知道文档位于一个索引内。*****
		...
		* 文档元数据	
         ...
         elasticsearch版本7.0，7.0版本有默认的type，8.0会彻底废弃
	      index、type将信息进行分层，利于管理。index可以理解为数据库，type理解为数据表，id相当于数据库表中记录的主键，具有唯一性。

         每个index下的type中，都可以去存储多个document。一个document里面有多个field，每个field就是一个数据字段
		   Index：索引，包含一堆有相似结构的文档数据，索引有一个名称,一个index包含很多document，一个index就代表了一类类似的或者相同的document。
      ...
      轻量搜索		不推荐直接向用户暴露查询字符串搜索功能，可读性差。
         1.一种是 “轻量的” 查询字符串 版本，要求在查询字符串中传递所有的参数，
            ...
            query string search
               ...
               简单快速的发出请求，检索信息；但是如果查询请求很复杂，很难构建就不适用，生产环境中，基本不用
   ...
   * 最重要的查询
      ...
      * ElasticSearch 5.0以后，String字段被拆分成两种新的数据类型: text用于全文搜索，会分词,而keyword用于关键词搜索，不进行分
            ...
            GET /megacorp/_mapping    索引，字段等
            GET /megacorp/_setting    分片，复制等
   ...
   * boot整合es
      ...
      * 常用操作
         * 新增数据
            等效于在索引Index中添加了一条json数据，然后index中有自定义的properties属性。
            ...
            json 目前用
               ...
               request.id(String.valueOf(twitter.getId()));//需要指定id
			...
			* 查询api相关
					...
					QueryBuilders
						 ...
						 QueryBuilders.termQuery(“supplierName”,param)
							...
							即两个重点：1. 不拆词 2. 与字段拆词结果词条（token）作equals比对
...
​	实时pull一个别人reset之前的操作，那么本地也是有记录的
   ...
   7. 本地git生成仓库，然后地址填远程，commit后push到远程，两地名称可以不一致，提交目录下得文件即可，最好保持一致

   8.git pull code 报错
      A .project file already exists in directory    删除目录下.project文件即可
...
### 模板设计
      模板方法(Template Method)模式定义：定义一个操作中的算法骨架，而将算法的一些步骤延迟到子类中，使得子类可以不改变该算法结构的情况下重定义该算法的某些特定步骤。它是一种类行为型模式。
...
## Zookeeper相关
   ...
   4. zookeeper结构
         zookeeper中的数据是按照“树”结构进行存储的,本身是一个树型的目录服务，支持变更推送，适合做注册中心.目录树结构，每个目录为znode	
         ...
         znode的结构
            主要属性	
               zxid：	每次变化都会产生一个唯一的事务zxid。通过zxid，可以确定更新操作的先后顺序(如果zxid1小于zxid2，说明zxid1操作先于zxid2发生)。zxid对于整个zk都是唯一的，即使操作的是不同的znode。
   ...
   6. ***所有的增删改查操作都是针对znode节点进行的***
         ...
         ZAB协议(忽略)
            是Zookeeper用来实现一致性的原子广播协议，该协议描述了Zookeeper是如何实现一致性的，分为三个阶段：
   ...
   10. 半数机制：集群中半数以上机器存活，集群可用。所以zookeeper适合装在奇数台机器上。
         Zookeeper虽然在配置文件中并没有指定master和slave。但是，zookeeper工作时，是有一个节点为leader，其他则为follower，Leader是通过内部的选举机制临时产生的

   11. zookeeper提供了文件系统和通知机制
         树状的目录结构，znode
         创建全局唯一路径，即使目录节点
   ...
   12. Zookeeper和Eureka的区别(忽略)

         zk：CP设计(强一致性)，目标是一个分布式的协调系统，用于进行资源的统一管理。
            当节点crash后，需要进行leader的选举，在这个期间内，zk服务是不可用的。

            集群环境，zookeeper是有一个节点为leader，其他则为follower
            ZK有一个Leader，而且在Leader无法使用的时候通过Paxos（ZAB）算法选举出一个新的Leader。
            这个Leader的目的就是保证写信息的时候只向这个Leader写入，Leader会同步信息到Followers。这个过程就可以保证数据的一致性

            当master节点因为网络故障与其他节点失去联系时，剩余节点会重新进行leader选举。
            问题在于，选举leader时间太长，30 ~ 120s, 且选举期间整个zk集群都是不可用的，这就导致在选举期间注册服务瘫痪,虽然服务能够最终恢复，但是漫长的选举时间导致的注册长期不可用是不能容忍的。

            zookeeper在选举leader时，会停止服务，直到选举成功之后才会再次对外提供服务，这时候服务不可用
         ...
         eureka：AP设计（高可用），目标是一个服务注册发现系统，专门用于微服务的服务发现注册。
            ...
            而Eureka的客户端在向某个Eureka注册或时如果发现连接失败，会自动切换至其它节点，只要有一台Eureka还在，就能保证注册服务可用（保证A），只不过查到的信息可能不是最新的（不保证C）。

            Eureka不会有类似于ZooKeeper的选举leader的过程；客户端请求会自动切换到新的Eureka节点；当宕机的服务器重新恢复后，Eureka会再次将其纳入到服务器集群管理之中；而对于它来说，所有要做的无非是同步一些新的服务注册信息而已。所以，再也不用担心有“掉队”的服务器恢复以后，会从Eureka服务器集群中剔除出去的风险了。

            Eureka因为没有选举过程来选举Leader，因此写的信息可以独立进行。因此有可能出现数据信息不一致的情况。但是当网络出现问题时，每台服务器也可以完成独立的服务，设计时就优先保证可用性
   ...
   13. Zookeeper集群中节点之间数据是如何同步的(忽略)
         采用发送快照和发送Diff日志的方式
         通过同步机制和两阶段提交机制来达到集群中节点数据一致


   14. 可以利用Zookeeper的临时节点和watch机制来实现注册中心的自动注册和发现
...
## RPC相关，服务化相关
   ...
   11. 熔断，降级，限流 
         * 熔断机制
               ...
               当服务A调用的某个服务B不可用时，上游服务A为了保证自己不受影响，从而不再调用服务B，直接返回一个结果，减轻服务AB的压力，直到服务B恢复

         * 降级
               降级多是大量请求且不能扩容进行的功能限制，可能针对某功能，也可能根据不同使用者。

               发现系统压力过载时，可以通过关闭某个服务，或限流某个服务来减轻系统压力，这就是服务降级。

         * 限流
               在⾼并发请求下，为了保护系统，可以对访问服务的请求进行数量上的限制，从而防止系统不被大量请求压垮
         ...
         * sentinel 和 Hystrix
            ...
            * sentinel
               https://baijiahao.baidu.com/s?id=1713371413877786952&wfr=spider    sentinel相关介绍
               
               sentinel常见问题，https://www.wenjiangs.com/doc/hzntyhp0
               
               BlockException 异常接口，有5个子类
               ...
               sentinel  控制台配置在消费端还是服务端

               系统保护规则对入口流量生效。入口流量指的是进入应用的流量(EntryType.IN)，比如 Web 服务或 Dubbo 服务端接收的请求，都属于入口流量。
               入口流量指的是进入应用的流量（EntryType.IN），通常作为 provider。比如 Web 服务或 Dubbo 服务端接收的请求。
               出口流量指的是从本资源调其它资源的流量（EntryType.OUT），通常作为 consumer。

               * Sentinel整合dubbo
                  实际推荐使用sentinel-annotation-aspectj组件整合
                  
                  Sentinel控制台
                     下载sentinel-dashboard.jar，直接执行java -Dserver.port=8080 -Dcsp.sentinel.dashboard.server=localhost:8080 -Dproject.name=sentinel-dashboard -jar sentinel-dashboard.jar

                     dashboard中,Sentinel使用懒加载，要请求一下接口才会显示出来
                        1、如果服务不被调用，sentinel控制台是无法感知到服务的
                        2、如果服务重启了之后，服务不被调用，控制台会一直认为服务实例处于下线状态，不会重连，之后服务调用后，服务主动连接sentinel控制台，控制台才会显示。
                        3、如果某个接口或者资源添加了@SentinelResource，但是没有被调用，控制台也是无法感知到该资源的存在的。
                           
                     流控配置目前都在provider端配置	 
                  
                  整合服务者和消费者
                     服务者和消费者的pom中需要引入相关依赖，pom引入同上，注意版本号要和sentinel.dashboard的版本一致
                        <dependency>
                        ...
                        <dependency>
                           <groupId>com.alibaba.csp</groupId>
                           <artifactId>sentinel-transport-simple-http</artifactId>					//将服务注册到控制台
                           <version>1.8.0</version>
                        </dependency>
                  
                  jvm启动参数添加，-Djava.net.preferIPv4Stack=true -Dcsp.sentinel.api.port=8720 -Dcsp.sentinel.dashboard.server=localhost:8080 -Dproject.name=xye-soa-core-require


                  @SentinelResource类似于Hystrix中的 @HystrixCommand 注解，是定义在方法上的注解。限流、熔断、报错了应该调用哪个方法

                  降级
                     选择以慢调用比例作为阈值，需要设置允许的慢调用 RT（即最大的响应时间），请求的响应时间大于该值则统计为慢调用。
                     单位统计时长默认1秒内请求数目大于设置的最小请求数目，并且慢调用的比例大于阈值，则接下来的熔断时长内请求会自动被熔断
               ...
               * @SentinelResource注解
                     暂时全部使用统一的fallback即可
                     本质上	@SentinelResource(value = "fallback", fallback = "fallbackErr", blockHandler = "blockHandlerErr")
                     还是捕获了对应的异常，FlowException，DegradeException
                     fallback：若本接口出现未知异常，则调用fallback指定的接口。
                     blockHandler：若本次访问被限流或服务降级，则调用blockHandler指定的接口。
                     
                     fallback
                        后备函数的名称，默认为空，可选项。
                        fallback指定方法名，fallbackClass指定类，本类无需指定
                     ...
                     blockHandler	
                        ...
                        blockHandler指定方法名，blockHandlerClass指定类，本类无需指定
                     ...
                     同时 fallback 和 blockHandler 都配置	
                     ...
                     fallback是针对方法出现异常了，则会进入fallback方法。blockhandler是针对流控设置，超出规则，则会进入blockhandler方法。
                     ...
                     ***配置sentinel的流控或者降级的资源名应和@SentinelResource的value值一样，而且不要再同时配置@GetMapping中value的资源名***
               
               
                     
                  实际消费端配置
                     这里注意SentinelResource的value和控制台中的资源名要一致
                     @RequestMapping("iindex.htm")
                     @ResponseBody
                     @SentinelResource(value = "com.xiaoyuer.soa.api.talents.IGroupInfoService",fallback = "fallbackErr",blockHandler = "blockHandlerErr")
                     public String groupInfoByEuserId(HttpServletRequest request) {
                        BaseModelInfo<TalentGroupBean>  groupInfoByEuserId = groupInfoService.getSingleGroupInfo(1366,1076,128,1,3);

                        System.out.println("123123");
                        return null;
                     }

                     public String blockHandlerErr(HttpServletRequest request,BlockException be) {
                        System.out.println("blockHandler ---------限流啦111");
                        return null;
                     }

                     public String fallbackErr(HttpServletRequest request) {
                        System.out.println("shutHandler --------熔断啦111");
                        return null;
                     }
            ...
            * sentinel 和 hystrix  
               ...
               dubbo不带熔断机制，不管调用失败多少次，消费者还是会继续进行调用。

               熔断的意义：Sentinel和Hystrix原则一致: 当检测到调用链路中某个资源出现不稳定的表现，例如请求响应时间长或异常比例升高的时候，则对这个资源的调用进行限制，让请求快速失败，避免影响到其它的资源而导致级联故障

               @HystrixCommand(fallbackMethod="backfix")，指定异常返回的处理方法
               @SentinelResource(value = "fallback", fallback = "fallbackErr", blockHandler = "blockHandlerErr")，指定异常返回的处理方法
               ...
               Sentinel和Hystrix区别
                  ...
                  hystrix已停止开发
                  Sentinel可以查看每个服务的接口的QPS数据，控制台功能齐全
                  
                  Sentinel 的侧重点在于：
                     多样化的流量控制(qps和并发线程数)，熔断降级(慢调用和异常)，实时监控和控制台,
                     
                  Hystrix的侧重点在于
                     以 隔离 和 熔断 为主的容错机制，超时或被熔断的调用将会快速失败，并可以提供 fallback 机制。

                  Sentinel的优势
                     限流
                        Sentinel可以以不同的运行指标（如 QPS、并发调用数、系统负载等）为基准，对资源调用进行流量控制，还可以对热点数据，黑白名单进行控制。
                        Hystrix没有限流策略，如果在微服务中使用限流策略，需要引用开源的限流算法，例如漏桶算法，令牌桶算法。
   ...
   24. rpc调用的流程
            在dubbo中是远程和本地使用同样的接口；
            1.要有对网络通信的封装，要对调用方来说通信细节是完全不可见的，
            2.网络通信要做的就是将调用方法的属性通过一定的协议（简单来说就是消息格式）传递到服务端；
            3.服务端按照协议解析出调用的信息；
            4.执行相应的方法；在将方法的返回值通过协议传递给客户端；客户端再解析；

            本地有对远程方法的描述，包括方法名、参数、返回值，
            在调用方式上又可以分为同步调用和异步调用；
            因为网络通讯都是传输的字节码，需要将我们使用的对象序列化和反序列化。


   25. 微服务架构下的事件驱动(忽略)
         可实现微服务的分布式数据管理
         事件驱动业务处理逻辑，其核心是，可靠的事件投递和避免事件的重复消费。

         在事件驱动架构中，当某件重要事情发生时，微服务会发布一个事件，例如更新一个业务实体。
         当订阅这些事件的微服务接收此事件时，就可以更新自己的业务实体，也可能会引发更多的事件发布，让其他相关服务进行数据更新，最终实现分布式数据最终一致性。
         可使用事件来实现跨多服务的业务交易，交易一般由一系列步骤构成，每一步骤都由一个更新业务实体的微服务和发布激活下一步骤的事件构成。

         事件驱动架构优点，可实现跨多个服务的事务实现，且提供最终数据一致性

         缺点，必须实现补偿事务以便从应用程序级故障中恢复，例如，如果信用检查不成功则必须取消订单;另外，应用必须应对不一致的数据，
         另一个缺点在于订阅者必须检测和忽略冗余事件，避免事件重复消费。
...
### 序列化相关
   ...
   5.  为什么要用序列化？
         ...
         当两个进程远程通信时，彼此可以发送各种类型的数据(以二进制序列的形式在网络上传送)。发送方需把Java对象转换为字节序列，才能在网络上传送；接收方则需要把字节序列再恢复为Java对象。
...
7. dubbo源码相关
      ...
      * dubbo的remoting
		...
		* 原理相关
         ...
         两个拦截器；主try包含两个从try，主事务commit包含三个参与者信息；
...
37. dubbo内网相关(忽略)
      ...
      使用到的端口都已经通过防火墙的配置，遇到无法获取生产者提供的服务，或者注册失败等信息的话，可能是因为内网IP造成的。
      配置的是公网IP，可是启动dubbo或者提供者的时候在dubbo-admin显示的确实内网IP。
      而生产者内网IP地址对于消费者不可达，所以消费者找不到生产者提供的dubbo服务，报出com.alibaba.dubbo.remoting.RemotingException: client(url: dubbo://[生产者内网IP]:20880/com.xxx.xxx.xxx.service.xxxService?...异常 

      解决方法是修改host文件，将这个内网ip改为公网ip即可
      ...
      另外一个是说Dubbo使用了一个DNS的地址，通过修改Dubbo服务器上的DNS配置【vim /etc/resolv.conf 】把服务名称改到DNS上去就可以了
         
      生产使用ip都是内网的ip
      <dubbo:registry id="soaRegistry" protocol="zookeeper" address="${zookeeper.address.soa}" />
      zookeeper.address.soa=192.168.8.112:2181?backup=192.168.8.113:2181,192.168.8.46:2181
...
1. 转二进制	// 序列化
      ...
      ```
        ...
        //序列化
         ObjectOutputStream objecOut=new ObjectOutputStream(new FileOutputStream("D:\\kkk\\ttt.txt"));
        ...
        //反序列化：
         ObjectInputStream objectIn=new ObjectInputStream(new FileInputStream("D:\\kkk\\ttt.txt"));
         Student student2=(Student)objectIn.readObject();
         System.out.println(student2);
         objectIn.close();

      序列化 ID
      虚拟机是否允许反序列化，不仅取决于类路径和功能代码是否一致，一个非常重要的一点是两个类的序列化 ID是否一致(就是 private static final long serialVersionUID)
...
9. 编码相关
      ...
      * 编码转换
         ...
         * 这种的参数中带url的就需要url全编码
            https://www.yu.com/xye-open/open-api/pay/forgetPwd.htm?userAccount=100000001366&wurl=https%3A%2F%2Fwww.yu.com%2Fxye-open%2Fopen-api%2Fpay%2Fwithdraw.htm%3Faccount%3D100000001366%26bankName%3D%E4%BA%A4%E9%80%9A%E9%93%B6%E8%A1%8C
                           
            encodeURIComponent("http://m.yu.com/setting/getSmsCode/payPwd.htm?name=中国&age=13")
            'http%3A%2F%2Fm.yu.com%2Fsetting%2FgetSmsCode%2FpayPwd.htm%3Fname%3D%E4%B8%AD%E5%9B%BD%26age%3D13'

            encodeURI("http://m.yu.com/setting/getSmsCode/payPwd.htm?name=中国&age=13")
            'http://m.yu.com/setting/getSmsCode/payPwd.htm?name=%E4%B8%AD%E5%9B%BD&age=13'		

            document.getElementById("forgetPayPwd").href=document.getElementById("forgetPayPwd").href + encodeURIComponent(decodeURIComponent(window.location.href));
      ...
      * 16进制字符Unicode，意思是“我是你爸爸”。这是汉字经过unicode编码产生的。
         汉字unicode编码后字符串，以\uxxxx的格式显示，之所以没有显示为汉字是因为没有解码，看到的\u6211是字符串格式，所以其实是\\u6211，第一个反斜杠转义了后面一个反斜杠，导致无法正确解码
         \u6211\u662F\u4F60
...
10. tcp三次握手的
		...
		1.客户端向服务端发送⼀个SYN。
         client发送server，TCP规定，SYN报文段（SYN=1的报文段）不能携带数据，但需要消耗掉一个序号
		2.服务端接收到SYN后，给客户端发送⼀个SYN_ACK。
         server接收数据，如果同意连接，则发出确认报文，这个报文也不能携带数据，但是同样要消耗一个序号
		3.客户端接收到SYN_ACK后，再给服务端发送⼀个ACK。
         client收到确认后，还要向服务器给出确认。此时，TCP连接建立，客户端进入ESTABLISHED（已建立连接）状态。TCP规定，ACK报文段可以携带数据，但是如果不携带数据则不消耗序号。当服务器收到客户端的确认后也进入ESTABLISHED状态，此后双方就可以开始通信了。
...
14. java.io操作类主要有4种(前两种属于数据格式，后两种属于传输方式)
		...
		IO 是面向流的，NIO 是面向缓冲区的
			...
			多路复用 IO 模式，通过一个线程就可以管理多个socket(一个线程不断去轮询多个socket的状态)，只有当socket 真正有读写事件发生才会占用资源来进行实际的读写操作。
			多路复用 IO 为何比非阻塞 IO 模型的效率高是因为在非阻塞 IO 中，不断地询问 socket 状态时通过用户线程去进行的，而在多路复用 IO 中，轮询每个 socket 状态是内核在进行的，这个效率要比用户线程要高的多。	
         多路复用的核心是，多个socket复用一个线程
   
         1.IO 是面向流的，NIO 是面向缓冲区的。
         2.IO 的各种流是阻塞的，NIO 是非阻塞模式。
...
18. netty入门学习(忽略)
   ...
   netty线程模型
      ...
      Reactor的作用相当于一个分发器，可以分发连接请求和处理IO请求

      实际上Netty线程模型就是Reactor模式的一个实现
      Reactor模式是基于事件驱动开发的，核心组成部分包括Reactor和线程池，其中Reactor负责监听和分配事件，线程池负责处理事件，
      而根据Reactor的数量和线程池的数量，又将Reactor分为三种线程模型:
      单线程模型 (单Reactor单线程)
      多线程模型 (单Reactor多线程)
      主从多线程模型 (多Reactor多线程)
...
21. NIO相关
      ...
      传统IO基于字节流和字符流进行操作，而 NIO 基于 Channel 和 Buffer(缓冲区)进行操作，数据总是从通道读取到缓冲区中，或者从缓冲区写入到通道中。
      Selector(选择区)用于监听多个通道的事件（比如：连接打开，数据到达）。因此，单个线程可以监听多个数据通道。

      Selector 类是 NIO 的核心类，Selector 能够检测多个注册的通道上是否有事件发生，如果有事件发生，便获取事件然后针对每个事件进行相应的响应处理。这样一来，只是用一个单线程就可
      以管理多个通道，也就是管理多个连接。这样使得只有在连接真正有读写事件发生时，才会调用函数来进行读写，就大大地减少了系统开销，并且不必为每个连接都创建一个线程，不用去维护多个线程，
      并且避免了多线程之间的上下文切换导致的开销。

      Netty 的 IO 线程 NioEventLoop 由于聚合了多路复用器 Selector，可以同时并发处理成百上千个客户端 Channel，
      由于读写操作都是非阻塞的，这就可以充分提升 IO 线程的运行效率，避免由于频繁IO阻塞导致的线程挂起。

      Netty采用的是异步通信模式，一个IO线程可以并发处理N个客户端连接和读写操作，这从根本上解决了传统同步阻塞IO一连接一线程模型，架构的性能、弹性伸缩能力和可靠性都得到了极大的提升。
    ...
    JAVA NIO 和 IO
      ...
      传统IO面向流操作，而NIO面向缓冲区操作

      IO 多路复用技术通过把多个IO的阻塞复用到同一个 select 的阻塞上，从而使得系统在单线程的情况下可以同时处理多个客户端请求。
      与传统的多线程/多进程模型比，I/O多路复用的最大优势是系统开销小，系统不需要创建新的额外进程或者线程，也不需要维护这些进程和线程的运行
      ...
      nio中的核心，Reactor
         当线程调用 channel.writeAndFlush()发送消息后，紧接着执行callback的 get()方法试图获取远程返回的结果。
         在get()内部，则使用 synchronized 获取回调对象callback的锁，再先检测是否已经获取到结果，
         如果没有，然后调用 callback 的 wait()方法，释放callback 上的锁，让当前线程处于等待状态
...
25. tcp/ip的四层
      ...
      3. 传输层
            此层中提供了节点间的数据传送服务，如传输控制协议(TCP)、用户数据报协议(UDP)等，TCP和UDP给数据包加入传输数据并把它传输到下一层中，这一层负责传送数据，并且确定数据已被送达并接收。
      4. 应用层
            应用程序间沟通的层，如简单电子邮件传输（SMTP）、文件传输协议（FTP）、网络远程访问协议（Telnet）等

26. 同一个内网段的机器才能相互访问，不通过网段的机器内网是访问不了的
...
3. Lambda 表达式
      ...
      代码实例
         ...
         List.sort((o1, o2) -> flag ? a.compareTo(b) : b.compareTo(a));
		   ...
		   List<JSONObject> dolistPage = new ArrayList<>();
         ...
         dolistPage.sort((a, b) -> new Long(b.getDate("doTime").getTime()-a.getDate("doTime").getTime()).intValue());
...
4. Stream流式编程
   ...
   stream流操作JSON对象
      而com.alibaba.fastjson下的JSONObject，JSONArray本质上来说其实是map和list

      1.取最后一条数据
         stream对象，先倒序，再findFirst取第一条即可
         JSONArray jsonArray = JSONArray.parseArray("[{\"name\":\"草根\",\"date\":\"2018-01-01\"},{\"name\":\"merry\",\"date\":\"2017-01-01\"},{\"name\":\"liming\",\"date\":\"2016-01-01\"}]");
         JSONObject jsonObject = (JSONObject)jsonArray.stream().sorted((a,b) -> -1).findFirst().get();
         System.out.println(jsonObject);

      2.清空json对象中的某个数组（该数组位于JSON对象内部多层）
         JSONObject jsonObject = JSONObject.parseObject("{\"1\": {\"name\":\"maple\",\"childrens\":[{\"name\":\"草根\",\"date\":\"2018-01-01\"},{\"name\":\"merry\",\"date\":\"2017-01-01\"},{\"name\":\"liming\",\"date\":\"2016-01-01\"}]}}");
      ...
      3.收集对象属性重新成一个数组
         将对象进行再次拼装以形成我们方便使用的数据状态：
         JSONArray jsonArray = JSONArray.parseArray("[{\"1\": {\"name\":\"maple\",\"childrens\":[{\"name\":\"草根\",\"date\":\"2018-01-01\"},{\"name\":\"merry\",\"date\":\"2017-01-01\"},{\"name\":\"liming\",\"date\":\"2016-01-01\"}]}}]");
      ...
      流中toCollection常用用法
         1.Collectors.toCollection(T :: new)
         ...
         2.groupingBy–分组
            ...
            outPut :  collect1 : {未成年=[{name=any, age=17}], 成年=[{name=joy, age=23}, {name=angle, age=20}]}
                  collect2 : {未成年={本地人=[{name=any, age=17}]}, 成年={本地人=[{name=angle, age=20}], 外地人=[{name=joy,

         3.其他的Collectors下面包含summingInt，averagingDouble，joining等类似方法，可以将元素在不同情景下统计为不同的形式(忽略)
...
6. 匿名内部类，Lambda，访问变量
      ...
      匿名内部类，变量的final，局部变量
         ...
         *****注意这里是访问局部变量才有final的限制，访问类的成员变量没有该限制。*****
...
7. 常用list，map操作
   ...
   //list元素变化后去重，map用作取一个集合   
   List<Integer> arrayList = new ArrayList<Integer>();
   arrayList.add(1);
   arrayList.add(2);
   arrayList.add(2);
	    	
	Set<Integer> collect = arrayList.stream().map(e -> e*2).collect(Collectors.toSet());
...
16. 系统相关命令
         cat /proc/meminfo       查看linux总内存
         top -p pid              查看某个pid使用内存情况
         lscpu                   查看linux的cpu情况
...
3. <? extends T>表示该通配符所代表的类型是 T 类型的子类。
   ...
   例 如 List<?>在逻辑上是List<String>,List<Integer>等所有List<具体类型实参>的父类
...
3. 常用配置和软件和系统问题,编辑器相关,
      ...
      * ecipse下错误: 找不到或无法加载主类，重新添加buildPath中的add源码即可，maven打包异常不覆盖，也适用
         ...
         自动构建项目， 虽然会减慢eclipes的速度， 但是保证不会因为之前的class文件使得项目报错。这个配置要打开的

      * 获取系统的换行符 system.getProperty("line.separator")
	      System.in.read(); 按任意键退出，用来模拟运行状态  

      * window环境，执行一个.bat文件，如若双击，错误闪退，如果是cmd进去执行，错误会显示出来
      ...
      * 项目版本，系统环境，系统版本，
         ...
         生产jdk版本
            ...
            其他都是openjdk，只有open是三方openjdk
               [kaifa@prd_xye-open1 ~]$ java -version
         ...
         本地maven版本
            Apache Maven 3.3.3 (7994120775791599e205a5524ec3e0dfe41d4a06; 2015-07+08:00)
            Maven home: D:\apache-maven-3.3.3\bin\..
            Java version: 1.8.0_212, vendor: Amazon.com Inc.
            Java home: C:\Program Files\Amazon Corretto\jdk1.8.0_212\jre
         ...
         生产发布机maven版本
            ...
            Java home: /usr/lib/jvm/java-1.8.0-openjdk-1.8.0.265.b01-0.el6_10.x86_64/jre
         ...
         tomcat版本
            生产tomcat版本    Implementation-Version: 9.0.37
            本地tomcat版本    8.5.42
         ...
         生产es版本
            ...
            "cluster_uuid" : "HQ1UFUWMTDadlYvLfhT2JA",
            "version" : {
               "number" : "7.2.0",
      ...
      * eclipse导入项目
            Eclipse导入项目时，报错No projects are found to import提示
            这是因为导入的文件里面缺少两个文件：.classpath 和 .project
         
            在这里三种方案：
               1、其他项目中直接复制这两个文件，然后更改一下“.project”文件中的项目名即可
               2、在Eclipse中新建一个项目，与需要导入的项目名一致，然后将项目代码文件直接复制到新项目中即可
               3、或者自己新建文件.classpath,.project

      * eclipse版本
            官网下载版本
               R Packages
                  release，发行版，标准版。
                  该版本意味“最终释放版”，在一系列测试版之后，最终有一个正式版本，对于用户而言，购买该版本的软件绝对不会错。一般下载这个就可满足我们的需求。
                  
               RC1 Packages
                  Release Candidate，正式发行候选版。RC1 就是正式发行版的候选版的第一版。
               M3 Packages
               M2 Packages
               M1 Packages

            Eclipse2021-06版本
               Eclipse2021-06(4.20）有一个问题，它已经为即将于2021年9月15日发布的Eclipse2021-09(4.21)修复
               Eclipse bug 574338-[content assist][regression]条件块中的变量没有内容辅助
               解决方法，
               1.您可以在使用内容辅助的位置之后添加;，
               2.或者等到下一版本或至少下一版本的第一个里程碑构建（2021年7月16日）。

               安装完eclipse2021版本，打开之后发现页面中间有条白色竖线
               Windows-General-点击Editors-点击Text Editors-Show print margin取消

            Eclipse2021-09版本
               实测，git，maven正常
               

      * eclipse中的git地址相关，拉取代码相关
         gitlab可以直接使用http的用户密码pull，以前是配置的ssh的秘钥

         show in repositories view 中，找到项目中的remotes中远程地址
...
4. 算法加密，加密算法，加密传输
   ...
   1. 三种加密算法
      ...
      一般系统对接会将公钥提供给对方，私钥自己留着用，这样对方客户端公钥加密后，自己的服务端私钥解密使用即可。
   ...
   4. https相关
         ...
         客户端请求https连接接--->服务端返回证书(公钥)--->客户端产生随机对称秘钥，使用公钥加密并发送--->服务端私钥解密得到对称秘钥--->两端实现通过对称秘钥加密的密文通信
         ...
         如果采用 HTTPS协议后仍需制作个http 可访问版，可通过 301 重定向自动跳入https版本

         HTTPS 在 HTTP 的基础上加入了 SSL 协议，SSL 依靠证书来验证服务器的身份，并为浏览器和服务器之间的通信加密。
         http 是超文本传输协议，信息是明文传输，https 则是具有安全性的 ssl 加密传输协议

         这套证书其实就是一对公钥和私钥(一把钥匙和一个锁头)，只是全世界只有你一个人有这把钥匙，你可以把锁头给别人，别人可以用这个
         锁把重要的东西锁起来，然后发给你，因为只有你一个人有这把钥匙，所以只有你才能看到被这把锁锁起来的东西。
         ...
         客户端在使用 HTTPS 方式与 Web 服务器通信时有以下几个步骤 
            ...
            客户端加密
               这个证书其实就是公钥（包含颁发机构，过期时间等信息），客户端首先会验证公钥是否有效，比如颁发机构，过期时间等等，如果发现异常，则会弹出一个警告框，提示证书存在问题。
               如果证书没有问题，那么就生成一个随机值，然后用证书对该随机值进行加密(把随机值用锁头锁起来，有钥匙才能看到被锁住的内容)
               这里传输的随机值实际上就是客户端的一个私钥
         ...
         SSL 证书申请3步骤(忽略)
            1、制作 CSR 文件
               所谓 CSR 就是由申请人制作的 Certificate Secure Request 证书请求文件，制作过程中，
               系统会产生 2 个密钥，一个是公钥就是这个 CSR 文件；另外一个是私钥，存放在服务器上。要制作 CSR 文件，申请人可以参考 WEB SERVER 的文档，一般 APACHE 等，
               使用OPENSSL 命令行来生成 KEY+CSR2 个文件，Tomcat，JBoss，Resin 等使用 KEYTOOL 来生成 JKS 和 CSR 文件，IIS 通过向导建立一个挂起的请求和一个 CSR 文件。

            2、CA 认证将 CSR 提交给 CA，CA 一般有 2 种认证方式：
               1、域名认证：一般通过对管理员邮箱认证的方式，这种方式认证速度快，但是签发的证书中没有企业的名称。
               2、企业文档认证：需要提供企业的营业执照，一般需要3-5个工作日。也有需要同时认证以上2 种方式的证书，叫EV证书，这种证书可以使 IE7 以上的浏览器地址栏变成绿色，所以认证也最严格。
            3、证书的安装
                  在收到 CA 的证书后，可以将证书部署上服务器，一般 APACHE 文件直接将 KEY+CER复制到文件上，然后修改 HTTPD.CONF 文件；TOMCAT 等，需要将 CA 签发的证书 CER文件导入 JKS 文件后，复制上服务器，然后修改 SERVER.XML；IIS 需要处理挂起的请求，将 CER 文件导入
...
12. 字符串比较，字符串相等判断
    ...
    String其长度是固定的，内容不能被改变和编译，虽然使用 + 号可以达到附加新字符串的作用，但 + 会产生一个新的String实例， 
    String是不可变类，字符串的连接操作(如a+b+c)总是通过生成新的Stirng对象来进行的
    如果尝试去修改，会新生成一个字符串对象，如String类，虽然是引用类型

    StringBuffer和StringBuilder是可变的
    StringBuffer是线程安全的，StringBuilder是线程不安全的，所以在单线程环境下StringBuilder效率会更高

    在内存中创建新的字符串对象，如果重复的对字符串进行修改，将会加大系统的开销，StringBuilder对象是一个可变的字符序列，大
    与StringBuilder类相似的还有StringBuffer类，StringBuffer类也是一个包含缓冲区的字符串对象，
...
19. 配置path的作用，环境变量(忽略)
      环境变量分为系统环境变量和用户环境变量,前者对所有用户起作用，后者只对当前用户起作用
      
      用户变量和系统变量到安装目录即可，这样path可以直接读取类似%MAVEN_HOME%的变量内容
      ...
      例如你要用opencv，那么你把opencv的bin目录加入到path变量下面，那么它就是系统环境变量，所用用户登陆，在命令行输入opencv都会有opencv的帮助信息出来。
...
26. HTTP 请求报文由 3 部分组成（ 请求行+请求头+请求体 ）
    ...
    web项目的标准结构：java+resource+webapp.这个默认的结构，一般不会去自己定义
      ...
      GET 提交，请求的数据会附在 URL 之后（就是把数据放置在 HTTP 协议头＜request-line＞中）
      POST 提交：把提交的数据放置在是 HTTP 包的包体＜request-body＞中。

      具体介绍(忽略)
         HTTP请求行格式：Method Request-URI HTTP-Version CRLF
               其中 Method 表示请求方法；
         ...
         HTTP 响应状态行格式：   HTTP-Version Status-Code Reason-Phrase CRLF
               其中，HTTP-Version 表示服务器 HTTP 协议的版本；
               Status-Code 表示服务器发回的响应状态代码；
               Reason-Phrase 表示状态代码的文本描述。状态代码有三位数字组成，第一个数字定义了响应的类别，
...
30. jdk版本
      ...
      jdk版本介绍
         jdk8版本问题
         简单概括，免费版本	JDK 8u202
                  收费版本	  openjdk			生产用的是openjdk version 1.8.0_242
               
         2019年4.16当天，Oracle发布了Oracle JDK的8u211和8u212两个版本（属于JDK8系列），从这两个版本开始收费
         当使用Oracle JDK 8u211及以上版本进行商业盈利性质时需要收费
         Oracle JDK的最后一个免费版本是2019年1.15发布的Oracle JDK 8u201和8u202

         Oracle JDK关键补丁更新(CPUs)包含安全漏洞修复和重要漏洞修复，**采用奇数编号**，建议及时升级到最新的CPU版
         Oracle JDK补丁集更新(PSUs)包含相应CPUs中的所有修复以及其他非重要修复，**采用偶数编号**，仅当受到CPUs版本之外的其他漏洞影响才应当使用该版本
         一般情况下只要下载奇数编号的最新版本更新即可，安全方面个人觉得还是8u202好点

         如果想用更新版本JDK功能特性，又不想收费授权，Oracle官方还提供了完全免费开源的JDK版本——OpenJDK
         Oracle在GPL许可下提供了开源的OpenJDK 发行版，最新版的OpenJDK一般都会紧跟对应版本的Oracle JDK，功能特性基本没有差异！

         Oracle JDK		https://www.oracle.com/java/technologies/downloads/
         Oracle OpenJDK	https://jdk.java.net/11/

         除了以上两种JDK，业界还有第三方提供的JDK，它们大都基于OpenJDK。比如Amazon Corretto。
         ...
         目前Amazon发布了Corretto的Corretto 8 Preview预览版本，它基于OpenJDK 8源码。

         下载地址：https://docs.aws.amazon.com/zh_cn/corretto/latest/corretto-8-ug/downloads-list.html

         安装openjdk
            这里不是exe安装版本，是一个压缩文件
            1.下载openjdk，是一个二进制文件目录
            2.下载之后将文件解压；主要路径不要有中文，空格等
            3. 配置系统环境变量；配置原则是最少配置原则，只简单配置1和2步骤即可，一般只配系统变量即可
               1.新建系统变量JAVA_HOME ,比如 E:\jdk1.8\java-se-8u41-ri
               2.编辑系统变量Path 的值，新建值：%JAVA_HOME%\bin
                     再当中添加"%JAVA_HOME%\bin","%JAVA_HOME%\lib"两项。项与项之间是分号(;)隔开,其中"JAVA_HOME\bin"是必须添加的。后者可以忽略
               3.(忽略)新建系统变量classpath值为：.;%JAVA_HOME%\bin;%JAVA_HOME%\lib\dt.jar;%JAVA_HOME%\lib\tools.jar （注意前边的.;点和分号），
               虽然从jdk1.5之后此步骤可以不做，但是为了避免之后出现什么问题，所以加了这个变量

         JDK环境变量主要有三个分别是：
            JAVA_HOME ：E:\java_web\Java\jdk-11.0.1(JDK安装路径)
            Path ：%JAVA_HOME%\bin;%JAVA_HOME%\jre\bin;（若已经有Path项，无须另外新建，直接在后面加，但需用;与前面已有的项分隔开）
            CLASSPATH ：.;%JAVA_HOME%\lib;%JAVA_HOME%\lib\dt.jar;%JAVA_HOME%\lib\tools.jar;
            java_home为jdk安装路径变量，方便其他变量可直接应用或方便修改切换jdk版本(当不定义时，其他变量使用具体的安装路径也可)。
            运行路径path变量记录的是各个程序所在的路径，系统根据这个变量的值来查找运行程序（各种命令），使得在运行的时候不用输入全路径名。
            类路径classpath环境变量通常用来记录当前路径和java类库所在的路径。在类库中包含java系统所提供的各种软件包，其中包括各个类和接口等。此环境变量是在使用”java Java类名“时自动搜索路径，若设置了此变量则会在指定路径下搜索类文件，所以可不进行配置，若设置建议在前边加上“.”，搜索当前路径。(1.5以前版本的JDK，需要配置，1.5以上版本已经不需要配置；并且JDK11的lib目录已经不再包含dt.jar和tool.jar文件)

         公司用的openj9， 自己电脑装的好像是oracle的jdk
...
41. 注解相关
      ...
      @Inherited 阐述了某个被标注的类型是被继承的(忽略)

      @Documented 描述-javadoc(忽略)
...
46. Websocket 电签  上传图片(忽略)
      ...
      WebSocket发生EOFException异常	
         聊天系统使用WebSocket进行消息传递，应用服务器使用两台做负载均衡，一台Web服务器部署Nginx做代理，
         登录系统之后总是会报java.io.EOFException异常，异常内容如下，
         从错误可以看出是WebSocket走了OnError方法，断开了连接，因为是使用的Nginx做代理，怀疑是Nginx将连接断开了，nginx默认60秒断开连接，
         尝试设置nginx的断连时间更长一点比如1小时。
...
54. 前端杂记，前端相关(忽略)
      ...
      5. js传参，前端传参
         ...
         * 3.标准参数模式
            ...
            $(“#form1”).serialize() 就是把表单的数据(input的name和value)拼成3类型的字符串
...
58. 基础知识
      ...
      * 手机号隐藏显示
         new StringBuilder(phoneNumber).replace(3, 7, "****").toString()

      * JAVA 四中引用类型
         强引用
            java中的引用默认就是强引用，
            任何一个对象的赋值操作就产生了对这个对象的强引用(对象赋给引用变量，该引用变量就是一个强引用)。

               比如，Object obj = new Object();
                     这里new了一个Object对象，并将其赋值给obj，这个obj就是new Object()的强引用。
                     强引用的特性是只要有强引用存在，被引用的对象就不会被垃圾回收。

            当一个对象被强引用变量引用时，它处于可达状态，它是不可能被垃圾回收机制回收的，
            即使该对象以后永远都不会被用到,JVM 也不会回收。强引用是造成Java内存泄漏的主要原因之一。

            强引用在jvm中始终都不会被回收，即便jvm内存不足，宁可抛出oom异常，也不会被回收。

            思考问题
               如果一个对象具有强引用，那么JVM必定不会回收这个强引用的对象，即使在内存不足的情况下，JVM宁愿抛出OutOfMemory错误也不会回收这种对象

               那么就有一个问题，Object obj=new Object()，obj作为强引用存在虚拟机栈中，而new Object()作为对象存在于堆中，
               当obj的作用域结束，对应的虚拟机栈消失，obj引用也同时消失，但new Object()对象却仍然存在于堆中，“JVM必定不会回收这个对象” ，那jvm不是很容易就OOM了吗？

               可手动设置obj=null，这样gc就会主动地将new Object()对象回收，但这样有点繁琐。

               实际上这句话缺少了一个前提，就是这个对象要和强引用还有关联，也就是在根搜索算法中，还和GC Roots相连接，这样jvm就一定不会回收强引用的对象。
         ...
         弱引用
            ...
            使用场景：一般用在集合类中，特别是哈希表

            弱引用需要用 WeakReference 类来实现，它比软引用的生存期更短，
            对于只有弱引用的对象来说，只要垃圾回收机制一运行，不管 JVM 的内存空间是否足够，总会回收该对象占用的内存。
         ...
         虚引用
            虚引用并不会决定对象的生命周期,主要用来跟踪对象被垃圾回收器回收的活动
            如果一个对象仅持有虚引用，那么它就和没有任何引用一样，在任何时候都可能被垃圾回收器回收。

            使用场景：一般用于对实现比较精细的内存使用控制。对于移动设备来说比较有意义

      * 队列，进行插入操作的端称为队尾，进行删除操作的端称为队头。


      * 正常情况下，当在try块或catch块中遇到return语句时，finally语句块在return方法之前执行，先finally后return。

      * JDK、JRE和JVM
         JDK，Java标准开发包，包括Java编译器、Java运行时环境，以及常用的Java类库等,用来开发Java程序
         JRE，Java运行环境，用于运行Java的字节码文件,也就是*.class文件
         JVM，Java虚拟机，是JRE的一部分，跨平台，负责运行字节码文件。

         JDK中包含了JRE，JRE中包含了JVM。
      
      * java -jar xye-manager.war 也可以直接执行
      ...
      *  idea项目编译
            idea项目在，右上角的edit configurations中配置jvm启动参数

            idea language level自动变化为5
      ...
      *  存放String 和obj不一样  字符串会有转义的东西，比如存在jsonObject中
	      Map<String, String> treeMap = new TreeMap<String, String>();
	      String reqArrayStr = JSONObject.toJSONString(reqArray);
			treeMap.put("reqList", reqArrayStr);
...
5. hystrix 服务容错保护
      ...
      熔断器hystrix
         ...
         使用Hystrix确实是可以实现刚刚我们在上面描述的这四个功能点.
...
## Docker相关
   ...
   vm 和docker的差别
      隔离级别，前者是系统级，后者是进程级
      镜像存储，前者gb-tb大小，后者kb-mb级(容器镜像大小通常只有几十兆))
   
      容器： 	应用层抽象，用于将代码和依赖资源打包在一起，一台机器上可运行多个容器，共享操作系统内核，但各自作为独立的进程在用户空间中运行 。与虚拟机相比， 容器占用的空间较少(容器镜像大小通常只有几十兆)，瞬间就能完成启动。
      虚拟机：物理硬件层抽象，用于将一台服务器变成多台服务器。 管理程序允许多个VM在一台机器上运行。每个VM都包含一整套操作系统、一个或多个应用、必要的二进制文件和库资源，因此占用大量空间。

      Docker容器提供了基于进程的隔离，而VM虚拟机提供了资源的完全隔离。容器使用宿主操作系统的内核，而虚拟机使用独立的内核。
   
      Size：
         虚拟机中ubuntu所占内存：24gb
         Docker容器中ubuntu镜像文件所占内存：80mb
      Startup：
         Docker在宿主机器的操作系统上创建Docker引擎，直接在宿主主机的操作系统上调用硬件资源，而不是虚拟化操作系统和硬件资源，所以操作速度快。
         一个是安装ubuntu的虚拟机和pull一个Docker的ubuntu镜像文件，Docker的启动速度是秒级的，vm是分钟级的。
      Integration：
         Docker的集成性要比VM好。整套镜像迁移
      ...
      docker run -m 256m 启动一个最多使用256m内存的容器，虽然进入到容器查看总内存跟物理内存一致，但是使用docker stats s1可以看到最多只能使用256m内存
...
6. docker相关指令
      ...
      docker stats 容器id  							// 查看容器内存使用情况  docker stats -a 查看所有容器情况
            ...
            0ff5d150080d   docker_pc_tomcat_103   1.04%     667.3MiB / 1.953GiB   33.37%    0B / 0B   20.6MB / 136MB   117
      ...
      **********
      ...
      docker混合命令记录
         ...
         docker tag  xye-tomcat:v1 192.168.6.138:5000/xye-tomcat:v1 // 打tag  必须符合 192.168.6.138:5000/xxxx
         docker push  192.168.6.138:5000/xye-tomcat:v1 // 推送到仓库
         docker pull 192.168.6.138:5000/mytomcat:v1   拉取地址
         http://192.168.6.138:5000/v2/_catalog 仓库地址
         ...
         #docker run -itd -h "docker_pc_tomcat" --add-host docker_pc_tomcat:192.168.6.139 --net=none -m 1500M  -v /usr/local/webapps/XiaoyuerProject:/usr/local/tomcat/webapps/ -v /usr/local/logs/pc:/usr/local/tomcat/logs  -v /etc/localtime:/etc/localtime  --name docker_pc_tomcat  192.168.6.138:5000/xye-tomcat:v1;sudo /usr/local/bin/pipework br0 docker_pc_tomcat 192.168.6.139/24@192.168.6.1
         #docker ps -a // 查看所有容器
         #docker stop docker_pc_tomcat  // 停止容器
         #docker start docker_pc_tomcat&&sudo /usr/local/bin/pipework br0 docker_pc_tomcat 192.168.6.139/24@192.168.6.1  // 再次启动容器，无须其它参数
         #docker rm -f docker_pc_tomcat // 删除容器  可以再次run并带参数
         #docker stats 容器id  // 查看容器内存使用情况  docker stats -a 查看所有容器情况
         #docker inspect docker_pc_tomcat // 查看容器元数据信息
         ...
         #//同步本地时间到 容器 -v /etc/localtime:/etc/localtime        添加到容器hosts文件 --add-host soa:192.168.6.141
         #docker run -itd   -h "docker_soa" --add-host soa:192.168.6.141 --net=none  -v /root/webapps/soa:/usr/local/java_project/ -v /root/logs/soa:/usr/local/java_project/logs/  -v /etc/localtime:/etc/localtime --name xye-soa soa:v1  && pipework br0 xye-soa 192.168.6.141/24@192.168.6.1;


7. 其他
         当使用/usr/local/tocmat/catalina.sh run的时候，其实是不会创建/usr/local/tocmat.logs/catalina.out文件的，
         因为这个文件只有在catalina.sh start的时候才会创建。而docker的初始执行命令就是sh /usr/local/tocmat/bin/catalina.sh run。

         Docker 将容器日志存储在其主要存储位置/var/lib/docker/. 每个容器都有一个特定于其 ID 的日志
         /var/lib/docker/containers/容器ID/容器ID-json.log
         
         这是它们的存储位置，采用JSON格式，不易读取，并且必须使用完整的容器ID。
         查看命令， 	docker logs -f e4bd48ef3103
         ...
         内存管理
            pre_pc_103的内存
            CONTAINER ID   NAME                   CPU %     MEM USAGE / LIMIT     MEM %     NET I/O   BLOCK I/O        PIDS
            0ff5d150080d   docker_pc_tomcat_103   0.84%     667.4MiB / 1.953GiB   33.37%    0B / 0B   20.6MB / 137MB   117

            ip为变量，看做103即可
            shell: docker run -itd -h "docker_pc_tomcat_{{ip}}" --add-host docker_pc_tomcat_{{ip}}:192.168.6.{{ip}} -m 2000M --net=none -v /usr/local/webapps/XiaoyuerProject:/usr/local/tomcat/webapps/ -v /usr/local/logs/pc/{{ip}}:/usr/local/tomcat/logs -v /etc/localtime:/etc/localtime --name docker_pc_tomcat_{{ip}} 192.168.6.138:5000/pre-pc-tomcat:v1;sudo /usr/local/bin/pipework br0 docker_pc_tomcat_{{ip}} 192.168.6.{{ip}}/24@192.168.6.1
            
            这里相当于只限制了最大内存，相当于运行中的进程内存在动态变化
...
## 系统优化方案，系统监控相关，性能调优相关
   ...
   7. 网站性能，安全防护，网站解析
         ...
         ddos攻击
            tcp三次握手的规则,这些计算机会向源 IP 发出 SYN+ACK 或 RST 包来响应这个请求。
            黑客所发送的请求包的源IP地址是被攻击主机的地址,这样受欺骗的主机就都会把回应发到被攻击主机处,造成被攻击主机忙于处理这些回应而瘫痪

            基本方法(忽略)
               1.关闭不必要的服务
               2.限制同时打开的 SYN 半连接数目,缩短 SYN 半连接的 time out 时间,限制 SYN/ICMP 流量
               3.正确设置防火墙，限定ip等认真检查网络设备和主机/服务器系统的日志。只要日志出现漏洞或是时间变更,那这台机器就可能遭到了攻击。
               4.限制在防火墙外与网络文件共享。这样会给黑客截取系统文件的机会,主机的信息暴露给黑客,无疑是给了对方入侵的机会。
               6.充分利用网络设备保护网络资源
               7.检查访问者的来源
   ...
   8. 了解系统总体架构，明确压力方向。具体的接口或者是模块的使用率。
         ...
         xss
            页面须检查是否可执行xss攻击，需要对输出进行转换，防止输入内容作为脚本执行。jsp可通过jstl转换
	



         csrf攻击
            csrf原理是cookies本身会自动携带
            若cookie中设置了HttpOnly属性，那么通过js脚本将无法读取到cookie信息，这样能有效的防止csrf攻击，窃取cookie内容，这样就增加了cookie的安全性。

            常见场景
               1.a登录划钱url，访问b的黑站，b中有改过参数的url，这时会跳转到a去划钱
               2.b站点弄个隐藏表单提交，实际也是页面的表单提交跳转，也是页面跳转
                  document.getElementsByClassName("foot-async-script")[0].innerHTML="<form id='aaa' action='https://www.xiaoyuer.net/homepage/ser/settings/save.htm' metdod='POST' display='none'><input type='text' name='accountNum' value='10001'/><input type='text' name='money' value='10000'/</form>";
                  document.getElementById('aaa').submit();

            token防御
                  正常表单页面会有token (在原网站会通过表单 浏览器发起含有token的请求)，其它网页发起的请求(没有表单 所以没有token) 会失败
                  这里的token，是系统再进入操作页面生成带过去的，存在session中的，然后页面操作需要将token传到后端去验证

                  <input type="hidden" id="xiaoyuerToken" name="xiaoyuerToken" value="4O4Q77JIFGVSE5RNQFXFSXFJG35ONY2V">

            <img src="http://www.xiaoyuer.net/homepage/ser/settings/save.htm?info=123">这种不行，没有页面跳转，无法附带cookie过去
   ...
   10. 性能排查，异常定位，性能测试
         ...
         内存分析工具
            ...
            jstat   	查看类加载，内存管理，gc情况(重点关注fullgc情况)
            jstack  	打印异常进程的堆栈信息 dump线程快照，定位死锁、阻塞，超时问题
   ...
   ​			QPS（TPS）= 并发数/平均响应时间
         ...
         对于还在正常运⾏的系统：
            首先，初步猜测频繁发送fullgc的原因，如果频繁发生fullgc但是又一直没有出现内存溢出，那么表示fullgc实际上是回收了很多对象了，所以这些对象最好能在younggc过程中就直接回收掉，避免这些对象进⼊入到老年代，对于这种情况，就要考虑这些存活时间不长的对象是不是比较大，导致年轻代放不下，直接进到了老年代，尝试加大年轻代的大小，如果改完之后，fullgc减少，则证明修改有效

            同时，还可以找到占CPU最多的线程，定位到具体的法，优化这个方法的执行，看是否能避免某些对象的创建，从而节省内存
   ...
   16. 服务器相关
         ...
         1. CPU个数，物理CPU
            ...
            服务器插槽上CPU的实际数量
            实际Server中插槽上的CPU个数,
            物理cpu数量，等于不同physical id的个数。
            
            服务器中“路”都是指服务器物理CPU的数量，也就是服务器主板上CPU插槽的数量。
            双路：指服务器支持2个CPU

         2. CPU的核心数（内核数）
            是硬件上存在着几个核心，一块CPU上面能处理数据的芯片组的数量
            比如，双核就是包括2个相对独立的CPU核心单元组，四核就包含4个相对独立的CPU核心单元组。依次类推。
            
            
         3. 线程数（逻辑处理器）
            线程数是一种逻辑的概念，也就是模拟出的CPU核心数。
            一般情况，我们认为一颗cpu可以有多核，加上intel的超线程技术(HT，Intel的cpu独有), 可以在逻辑上再分一倍数量的cpu core出来
            例如，可以通过一个CPU核心数模拟出2线程的CPU，也就是说，这个单核心的CPU被模拟成了一个类似双核心CPU的功能
            如果没有超线程技术，一个CPU核心对应一个线程。对于AMD的CPU来说，只有核心数的概念，没有线程数的概念。 核上的线程会共享核资源
            决定了服务器并行(同时)处理任务的能力
         ...
         这里是4路8核的物理cpu，不开HT，每台VM最多能分配32个vcpu。
         这32个可以是1个socket、32核。也可是32个socket、每个socket里面有1个虚拟内核。但不管怎么分配，都不会超过物理内核数量。
         ...
         比如，Linux服务器有4个CPU，每个CPU有4个核心，每个核心有4个超线程(需要支持开启超线程技术，intel超线程技术(HT))，一共有4*4*4=64个线程，也称为64个逻辑cpu。
         ...
         lscpu，prd pc
            ...
            公司办公室机房配置
               ibm x3650m4				   四核四线程
               dell poweredge r420		四核四线程		
            
            prd pc(6个逻辑cpu),netpay(4个逻辑cpu)  用的cpu， Intel(R) Xeon(R) CPU E5-2620 v3 @ 2.40GHz			6核12线程   
...
## Jvm相关，虚拟机相关
   ...
   1. 基础的概念
      ...
      * java堆(heap)：
         是jvm中所管理内存中最大的一块，是所有线程共享的内存区域，jvm启动时创建。用来存放对象实例。堆是线程共享。
         所有对象的实例都要在堆上分配
         ...
         比例分配 共10分
            新生代（eden,from,to，比例是8/10,1/10,1/10）占1/3，老年代占2/3
      ...
      * java虚拟机栈
         ...
         基础数据类型直接在栈空间分配(函数中局部变量int b，在栈中为b开辟空间)，包装类如Integer等存放在堆中
      ...
      * 方法区：
         ...
         存放class相关信息和常量池相关()，如类名、访问修饰符、常量池、字段描述、方法描述(class信息,string常量和static变量保存在方法区),线程共享,比如100，"hello"和常量都放在常量池中
   ...
   2. 内存溢出
         ...
         * 虚拟机栈和本地方法栈溢出
         ...
         内存泄漏场景：
            ...
            (4) 外部类实例对象的方法返回一个内部类的实例对象，内部类长期引用，外部类虽然不用了但被内部类持有，回收不了

            (5) 线程池中使⽤ThreadLocal会造成内存泄漏
                  因为当ThreadLocal对象使⽤完之后，应该要把设置的Entry<key，value>进⾏回收，但线程池中的线程不会回收，⽽线程对象是通过强
                  引⽤指向ThreadLocalMap，ThreadLocalMap也是通过强引⽤指向Entry对象，线程不被回收，Entry对象也就不会被回收，从⽽出现内存泄漏，
   ...
   3. 对象访问（句柄访问，指针访问）
         ...
         对象的访问 
            ...
            一般来说，一个Java的引用访问涉及到3个内存区域：JVM 栈，堆，方法区。
            以最简单的本地变量引用：Object obj = new Object()为例：
            • Object obj 表示一个本地引用，存储在 JVM 栈的本地变量表中，表示一个 reference类型数据；
            • new Object()作为实例对象数据存储在堆中；
            • 堆中还记录了Object类的类型信息（接口、方法、field、对象类型等）的地址，这些地址所执行的数据存储在方法区中；

   4. jvm判断对象是否存活，
         
         可达性分析，gc root，根搜索算法（常用）
   ...
   7. finalize()：(忽略)
         ...
         如果对象在可达性分析中没有与 GC Root 的引用链，那么此时就会被第一次标记并且进行一次筛选，筛选的条件是是否有必要执行 finalize()方法。这里不细看
   ...
   8. 内存分配与回收策略
         ...
         一般情况下，触发老年代 GC的时候不会触发 Minor GC,但是通过配置，可以在 Full GC 之前进行一次 Minor
         GC 这样可以加快老年代的回收速度

         static变量的回收
            静态变量是属于类的不是属于实例对象的；静态变量存储在方法区；这个区域不是用于存储那些从老年代存活下来的对象，按照分代属于持久代；这个区域也可能发生GC(条件非常严苛)。
            发生在这个区域的GC事件也被算为 Major GC 。
            静态变量很少会被GC回收的
            Major GC 是清理永久代。
   ...
   9. jvm的性能监控域处理工具   
      ...
      小鱼儿系统中的内存分配
         ...
         start() {
            ...
            echo -n "Starting ${APP_NAME}: "
            echo 
            nohup java -Dloader.path=$XYE_LIB_DIR,$OTHER_LIB_DIR -Xms2048m -Xmx2048m -XX:PermSize=256M -XX:MaxPermSize=256M -jar $JAR_HOME_DIR$APP_NAME > /dev/null 2>&1 &
            fi
         }
         ...
         prd_job中是，JAVA_OPTS="-server -Xms1600m -Xmx1600m -XX:MaxMetaspaceSize=400m"
         ...
         设置元空间大小时，建议加上-XX:MaxMetaspaceSize参数，给个范围限制，和-XX:MetaspaceSize参数值一样，一般设置成256M即可，避免元空间占用的内存一直增长。
   ...
   10. jvm的类加载机制：jvm把描述类的数据从class文件加载到内存，并对数据进行校验，转换解析和初始化，最终形成可被jvm直接使用的java类型，	
         ...
         * 加载：
               ...
               1.获取类的二进制字节流，2.将其代表的静态存储结构转化为方法区的运行时数据结构，3.在堆中生成代表该类的class对象，作为方法区这些数据的访问入口。
         * 验证：
               验证这个 class 文件，包括文件格式校验、元数据验证，字节码校验等。
               确保class文件的字节流中包含的信息符合当前虚拟机的要求，并不会危害jvm自身安全
         * 准备：
               为类变量进行内存分配和初始化零值的过程
               在方法区中，为类变量(被static修饰的变量，不包括实例变量)分配内存并设置类变量的初始值，实例变量将会在对象实例化时随着对象一起分配在java堆中，
               而且此时类变量是赋值为零值，即 int 类型的初值为 0，引用类型初值为 null，而不是代码中显示赋值的数值。

         * 解析：
               将符号引用转化为直接引用（指针引用）
               jvm将常量池内存内的符号引用替换为直接引用的过程
         * 初始化：
               开始执行构造器的代码
               真正开始执行类中定义的java程序代码(或者说是字节码)
               这时执行一些静态代码块，为静态变量赋值，这里的赋值才是代码里面的赋值，准备阶段只是设置初始值占个坑
         ...
         符号引用和直接引用(忽略)
            ...
            如说某个方法的符号引用，如：“java/io/PrintStream.println:(Ljava/lang/String;)V”。里面有类的信息，方法名，方法参数等信息。

            而直接引用是JVM（或其它运行时环境）所能直接使用的形式。
            直接引用就是偏移量，通过偏移量虚拟机可以直接在该类的内存区域中找到方法字节码的起始位置。

            当第一次运行时，符号引用，要根据字符串的内容，到该类的方法表中搜索这个方法。
   ...
   11. 双亲委派机制(防止重复加载同一个.class)
      **双亲委派机制，就是一个类只能被一个类加载器加载。**
         采用双亲委派的一个好处是比如加载位于 rt.jar 包中的类 java.lang.Object，不管是哪个加载器加载这个类，最终都是委托给顶层的启动类加载器进行加载，这样就保证了使用不同的类加载
         ...
         *****
         ...
         所有的加载请求都应该传送到启动类加载其中，只有当父类加载器反馈自己无法完成这个请求的时候（在它的加载路径下没有找到所需加载的Class），子类加载器才会尝试自己去加载
         ...
         因为在JVM 中一个类用其全名和一个加载类 ClassLoader的实例作为唯一标识，不同类加载器加载的类将被置于不同的命名空间。
   ...
   12. 垃圾回收的算法
         ...
         分代算法
            新生代中：主要是用来存放新生的对象。一般占据堆的1/3空间。由于频繁创建对象，所以新生代会频繁触发MinorGC进行垃圾回收。
               对象优先在Eden区分配
               Eden区：
                  Java新对象的出生地(如果新创建的对象占用内存很大，则直接分配到老年代)。该区域内存不足会触发minorGC
               SurvivorTo：
                  保留了一次MinorGC过程中的幸存者。
               SurvivorFrom：
            ...
            minorGc的核心动作是：复制-->清空-->互换,

               复制：eden、survicorFrom 复制到 SurvicorTo，年龄+1，默认情况下年龄达到15的对象会被移到老年代
                  
                  首先，把Eden和SurvivorFrom区域中存活的对象复制到SurvicorTo区域（如果有对象的年龄以及达到了老年的标准，则赋值到老年代区），同时把这些对象的年龄+1（如果SurvicorTo不够位置了就放到老年区）

               清空：清空 eden、survicorFrom
                  
                  然后，清空 Eden 和 SurvicorFrom 中的对象；

               互换：SurvicorTo 和 SurvicorFrom 互换
                  
                  最后，SurvicorTo 和 SurvicorFrom 互换，原 SurvicorTo 成为下一次 GC 时的 SurvicorFrom区。
   ...
   13. 垃圾回收器
         垃圾回收器我们可以把它分为这三类，
            第一类叫串行的垃圾回收器，
               Serial 和 SerialOld，
            第二类叫吞吐量优先的垃圾回收器，
               ParallelGC 和 ParallelOldGC
            第三类叫响应时间优先的垃圾回收器
               CMS

         Serial（串行）收集器
         ParNew收集器其实就是Serial收集器的多线程版本
         Parallel Scavenge收集器，关注点是吞吐量（高效率的利用CPU）。

         Serial Old收集器，Serial收集器的老年代版本，它同样是一个单线程收集器
         Parallel Old收集器，Parallel Scavenge收集器的老年代版本
         CMS收集器，并发收集、低停顿。CMS等垃圾收集器的关注点更多的是用户线程的停顿时间
         G1收集器，面向服务器的垃圾收集器
   
   14. 定义类就是定义类中的成员，包括成员变量（属性）和成员函数（行为）
         成员变量属于类，在堆中
         局部变量属于方法，在栈中
         new 一个对象的过程：
