...
### 基本知识点
   CAP定理
      一致性(Consistency) ： 强一致性,    客户端知道一系列的操作都会同时发生(生效)，数据一致更新，所有数据变动都是同步的。最终一致和强一致性  这里一致性用最终一致即可。在数据同步过程中，是不能对外提供服务的，不然就会造成数据不⼀致，所以强⼀致性和可⽤性是不能同时满⾜的

      可用性(Availability) ： 高可用性,   每个操作都必须以可预期的响应结束，分布式系统对外要保证可⽤。
      分区容错性(Partition tolerance) ：分布式容忍性, 分布式系统内即使出现单个组件节点无法可用,操作依然可以完成
...
## RPC相关，服务化相关
   ...
   11. 熔断，降级，限流 
         ...
         * 限流
               在⾼并发请求下，为了保护系统，可以对访问服务的请求进⾏数量上的限制，从⽽防⽌系统不被⼤量请求压垮
