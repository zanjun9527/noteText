...
## 后续要看的内容
   ...
   整理进度 刚到，线程变量Threadlocal，ThreadLocal相关
...
2.  引入xml配置
   在启动类上加上@ImportResource({"classpath:config/spring-mvc.xml"})即可注入xml配置 ，配置spring-mvc 建议使用官方的xml头文件
...
13. 杂项
      * springboot中使用jsp，支持需要添加tomcat-jsp解析包
            ```
               <dependency>
...
#### lock锁和synchronized同步锁，Lock 和 synchronized
   ...
   4. Lock 和 synchronized 的区别
		...
		2. Lock 是一个接口(API级别)，是java类，而 synchronized 是 Java 中的关键字(JVM 级别)，是内置的语言实现。
